...
- 2026-08-01 `src/radiant/atmosphere/tabulated.py:421,530` and `src/radiant/atmosphere/modtran.py:1611` — CU-306 moved the **interpolated** backend's τ resample into log-τ, but `TabulatedAtmosphere` and `ModtranAtmosphere` still carry τ onto the chain grid with a linear-in-τ `SpectralData.resample`, so the same stored MODTRAN column now yields slightly different τ depending on which backend serves it (measured on the shipped ladders: up to ~1.5 % rel at a 200-point MWIR chain grid). No operation-order defect there (one resample, nothing to commute), so it is a backend-consistency question, not the CU-306 bug — but **promotion candidate: results-affecting** if the owner wants one convention across backends. (origin: CU-306, branch `atmo/cu-306-logtau-resample`)

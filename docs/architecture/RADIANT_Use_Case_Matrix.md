...
| ε(λ) door (`emissivity_path`) paired with a rival surface | `source.target.emissivity_path` set together with scalar ε, ρ-family, S8, S10, S11 or S12 | Raise `ParameterBoundsError` naming the rival | **Implemented (CU-318, 2026-08-02)** — extracted from `source/_inferrer.py::_load_emissivity_on_grid` into `target_spec.py::check_emissivity_path_conflicts` and registered last in `validate_target_spec` (the position the ε(λ) door occupies in dispatch). The pair unique to this door — `emissivity_path` + scalar `source.target.emissivity` — reached only `evaluate()` before; it is now refused at both entry points with identical text. Its other nine rivals each open a door that dispatches **first**, so `evaluate()` never reaches this guard and discards the ε(λ) surface in silence (Rule 17, same class as the S11/S12 row above); the seam refuses them, and closing the asymmetry the other way needs the CU-293 owner ruling extended to those doors |

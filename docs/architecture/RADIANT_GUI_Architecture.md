...
| **Calibration** | Phase 0 shell (Gap 120, ADR-0012) | **[exists — shell only]** The stage joined the chain 2026-09-06, so the strip carries its chip ("NUC · bias") and `STAGE_COMPOSITIONS` a minimal composition (title + themed note; `calibration.*` parameters edit via the left parameter panel like any stage). The ratified full screen — scheme-selector card (the Gap 117 `architecture_switch` pattern), contextual drift/cal-source groups, precision-vs-accuracy readout — is `docs/plans/Calibration_Model_Plan.md` §7, lands at plan Phase 3, and is subject to the owner live-review loop before merge |

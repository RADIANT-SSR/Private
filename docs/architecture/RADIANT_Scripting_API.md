...
| `cs.clone()` | An independent copy of the whole set: cloned base, copied configured table, wavelength-point overrides (per configuration **and** shared), `active`/`baseline`. The set-level counterpart of `Sensor.clone()`; nothing is shared afterwards in either direction. Use it for thread isolation (the GUI hands its evaluate-all worker `cs.clone()` taken on the GUI thread) or before a destructive what-if. Rebuilding a copy from the public accessors is **not** equivalent — the wavelength-point overrides have no read accessor (CU-210), so a hand-built copy would silently evaluate on the default grid. |

...
**Comparison range (CU-345, owner-ratified 2026-09-07):** the `[:nyquist]` slice above is now literal — `PerformanceStage` passes `nyquist_cycles_per_mrad` and the check stops at detector Nyquist. Before the fix the code compared the first half of the PSF frequency grid, whose reach grows with `psf_oversample` and landed 4–8× past Nyquist on shipped configs; on the 9.4 OLI-2 bands the pixel-sinc sidelobe discretization there peaked at 2.02e-2 (a spurious FAILED on a flagship scenario) while the sub-Nyquist worst case is 5e-3 — comfortably inside tolerance. A genuinely missing degradation term still shows below Nyquist, so the cap loses no detection power.

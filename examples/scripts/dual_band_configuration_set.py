...
def print_physics_discussion(study: ConfigurationSet, run: ConfigSetRunResult) -> None:
    ...
    print()
    print("  Aliasing rows, read alongside the above: sampling replicates the")
    print("  pre-sampling spectrum at f_s = 2 x f_Nyquist, so at f_Nyquist the")
    print("  k = -1 replica lands back on f_Nyquist and mtf_folded_at_nyquist")
    print("  is twice mtf_at_nyquist. MWIR: 0.5334 = 2 x 0.2667, alias fraction")
    print("  0.5000 -- half the apparent contrast at Nyquist is folded-in")
    print("  above-Nyquist scene content. LWIR: 9.7e-16, i.e. zero -- the optics")
    print("  cut off below Nyquist, so there is nothing to alias, and its")
    print("  alias_fraction_at_nyquist reads exactly 0. That zero is reported,")
    print("  not computed: the ratio (folded - optical)/folded is evaluated only")
    print("  where the folded MTF exceeds 1e-9 of its DC value, below which both")
    print("  terms are round-off (this band used to print 0.944, which was float")
    print("  noise divided by float noise).")
    print()

...
| **Status** | **RESOLVED** — both halves. Half (a) closed 2026-07-29 (commit `5c0f3dd`, CU-254): the sky is now rooted at the **sensor** and spans the whole LOS, so the `h_tgt ≥ h_atm_top` early return is gone and the provisional VIS/NIR `UserWarning` fires for this scene class; `sky_radiance.warn_if_scattered_sky_provisional` is public so the near-horizon branch applies the same gate. The `_sky_source_radiance` symbol named below is now `_sky_radiance_at_aperture`. Half (b) closed 2026-08-01 (CU-260): the single-scatter species split is taken at the segment's **lower endpoint** instead of its arithmetic mean altitude, so the GEO case no longer underflows. Re-measured 2026-08-02: the GEO daylight run at θ_s = 60° now gives observer-leg L_path = **3.2774 W/m²/sr/µm** (was 1.3637e-18) with **1** provisional sky warning raised, and the 20 km comparison probe gives 3.0705 W/m²/sr/µm — the two agree to 7 %. |
| **Severity** | High |
...
| **Status** | **CLOSED 2026-08-01 as documentation** (CU-257, owner ruling; no guard change). Two corrections to the original write-up below. (1) **The guard never fires on shipped 10.3 at all**: CU-256 forbids a declared area alongside the point-intensity door, so the scenario's `angular_extent_rad` is 0 and `_validate_psf_regime_consistency` short-circuits before the ratio test (the non-finite/non-positive-extent early return in `_validate_psf_regime_consistency`). The dead end was reached only by the hand-built shape+albedo probe named in **Found in**. (2) **The measured figures below are wrong** — see the corrected block. The resolution is declaration guidance, not a guard change: a seeing-limited but physically unresolved target is correctly declared `sub_pixel`, which passes today. Guidance and full numbers: `docs/architecture/RADIANT_Use_Case_Matrix.md` §1.1.1. |
| **Severity** | Medium (as originally assessed) |
...
| **Description (corrected 2026-07-30, re-measured on the shipped config)** | The 4–15× turbulence broadening was never measured on this configuration; the true factor is **2.32×**. At the shipped 10.3 geometry (slant range 739.156 km, band centre 0.650 µm, r₀ = 19.82 cm): seeing FWHM (0.98 λ/r₀) = **3.21398 µrad** (0.663 arcsec); optics-only PSF FWHM = **1.52368 µrad**; platform PSF FWHM (turbulence + jitter + smear) = **3.54066 µrad**. A 1 m² target at that range gives `√A_t/d` = **1.35296 µrad**, i.e. **0.888×** the optics-only FWHM (not 1.305× — that would need `A_t` ≈ 2.16 m²) and **0.382×** the turbulence-inclusive FWHM. Both are far above the `0.1` point-source ceiling, so referencing the guard to the degraded PSF **would not open the point-source door either**: for 1 m² it opens only beyond ≈ 6 563.06 km (optics-referenced) or ≈ 2 824.33 km (turbulence-inclusive). The "unresolved by 2.4×" claim does survive — seeing FWHM ÷ target extent = **2.375×**. Both ratios are ≫ the `0.01` sub-pixel lower bound, so the `sub_pixel` branch passes with ~2 decades of margin. |
...
| **Resolution** | Declare `sub_pixel` (with `A_t` + `shape`) for a seeing-limited unresolved target — the shape/area door is open today by that route, and the seeing enters the radiometry through the fully degraded PSF that `PlatformStage` feeds into EE_box (Rule 9). The `0.1·PSF_FWHM` point-source criterion is optics-referenced by design and stands unchanged. |
| **Impact** | As re-scoped: only the `point_source` *declaration* is refused for a shape+albedo LEO target; `sub_pixel` is not. Note that re-referencing the guard to the degraded PSF would be bidirectionally results-affecting — the same 2.32× widening raises the `0.01·PSF_FWHM` sub-pixel lower bound, newly refusing targets between `0.01` and `0.0232` of the optics FWHM. |
| **Fix location** | None — closed as documentation. `optics/stage.py::_validate_psf_regime_consistency` is unchanged; its docstring now records the optics-referenced intent. |
| **Effort** | N/A (documentation). |
| **Rerun after fix** | N/A. A future 10.3 variant with a shape+albedo LEO target declares `sub_pixel`. |
...
| **Status** | **RESOLVED 2026-08-01 (CU-261)** — the vacuum branch now publishes an exactly-zero sky radiance instead of `None`, so the `SkyBackground` assembly no longer refuses it. Re-measured 2026-08-02: the true-vacuum run (telescope raised to `h_atm_top` = 100 km) completes and returns `max |τ_up − 1| = 0.000e+00`. |
...
| **Status** | **RESOLVED 2026-07-28 (CU-253)** — the Rayleigh term is now read as an optical depth rather than a km⁻¹ volume extinction. Re-measured 2026-08-02 at the 900 m site, zenith: RADIANT τ(0.55 µm) = **0.8894** → **0.127 mag/airmass**, inside the published 0.12–0.20 band, so cross-check (b) now returns PASS. This scenario's nominal SNR moved 186.89 → 232.38 (**+24 %**) and band-mean τ_up 0.5330 → 0.8397. |
...
| **Workaround** | None was available before the fix. The scenario reported RADIANT's numbers as-is and stated that τ (and hence SNR) was pessimistic by ≈ 1.8× at the nominal geometry, with trends unaffected. No workaround is needed now. |

# GUI scripting-window script — Scenario 6.3: Noise-model verification — reference sensor
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/6.3_noise_model_verification.gui.yaml
...
#
# NOTE: Runner's module-level verification sensor. GUI baseline widens the ADC to 21 bits (from 14) so the bright MWIR signal (~1.6 Me-) fits the ADC range instead of clipping; gain stays 1.0 e-/DN, so every verified noise term — quantization = gain/sqrt(12) = 0.289 e- included — is bit-identical to the runner (CU-170).
#
...
# (headless smoke test): python scripts/gui_console_6.3_noise_model_verification.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "6.3_noise_model_verification.gui.yaml"
    )
...
print("=== Scenario 6.3: Noise-model verification — reference sensor ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 6.3: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

...
and the Source / Optics / Detector Inputs forms (Phases PS-1/2/3). The value shows in the
row's **display unit**, sharing the parameter panel's session display-unit store so a unit
chosen on any surface agrees. Each accepted edit re-emits :attr:`parameterEdited`, so the
host debounces a full re-evaluation and the in-band spectrum + the Outputs readout refresh
(edit-and-watch).
...
# The band-defining filter edges (label, dot-path). Bounds/units/description all come from
# the live schema (never transcribed) — only the human label + the grouping is a literal
# here (CU-120, tracked with the geometry/source/optics/detector manifests).
_FILTER_FIELDS: Final[tuple[tuple[str, str], ...]] = (
    ("Filter min (short edge)", "spectral_integration.filter_min_um"),
...
class SpectralIntegrationInputsForm(QWidget):
    """The Spectral-Integration inputs card: schema-driven :class:`FieldRow` per knob.
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self.setObjectName("spectralIntegrationInputsForm")
        ...
        # optics/detector card), so these inputs sit in an identically-styled card.
        card = QWidget(self)
...
__all__ = ["SpectralIntegrationInputsForm"]

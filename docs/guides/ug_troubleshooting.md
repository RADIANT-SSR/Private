...
| a loaded configuration carries two disagreeing doors of one geometry family (an edit cannot create this state: a second door is refused inline at the editor — *Two viewing doors are set …* — and the family's selector on the Geometry workspace is the switch) | `Geometry conflict — set exactly one input per family; the tinted card on the Geometry workspace is the one to fix (see Messages; the previous result is shown, stale)` |

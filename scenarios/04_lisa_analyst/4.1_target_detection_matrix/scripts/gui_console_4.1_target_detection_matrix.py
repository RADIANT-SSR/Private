# GUI scripting-window script — Scenario 4.1: Target detection matrix — Sensor A (MWIR smallsat)
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/4.1_target_detection_matrix.gui.yaml
...
# (headless smoke test): python scripts/gui_console_4.1_target_detection_matrix.py
...
print("=== Scenario 4.1: Target detection matrix — Sensor A (MWIR smallsat) ===")
...
def _sweep_and_plot():
    center = 0.18  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 4.1: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

# GUI scripting-window script — Scenario 3.1: ISR pass planning — 30 deg off-nadir baseline
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/3.1_isr_pass_planning.gui.yaml
...
#
# NOTE: Nominal 30 deg off-nadir look.
#
...
# (headless smoke test): python scripts/gui_console_3.1_isr_pass_planning.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "3.1_isr_pass_planning.gui.yaml"
    )
...
print("=== Scenario 3.1: ISR pass planning — 30 deg off-nadir baseline ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 3.1: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

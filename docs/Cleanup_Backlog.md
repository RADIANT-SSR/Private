...
5. ✅ CI workflow (`.github/workflows/ci.yml`) — landed 2026-04-26 (slice 5 of 5). Four jobs: (a) `static` runs `ruff check src/`, `mypy --strict src/radiant/core src/radiant/api`, and `lint-imports --config pyproject.toml`; (b) `fast-tests` runs level0 then level1 sequentially (so a level0 failure short-circuits before paying for level1); (c) `integration-tests` runs level2 with `needs: fast-tests` so it only fires when level0+level1 passed; (d) `golden` runs the snapshot regression on push-to-main + `workflow_dispatch` only (PRs don't run goldens because legitimate physics changes routinely move snapshot values, and the noise would mask real signals). All jobs run on `ubuntu-latest` with Python 3.11, pip cache enabled, and `concurrency: cancel-in-progress` so superseded commits don't tie up minutes. `ruff format --check` is **not** included in this slice — pre-flight reports 160 of 346 source files would be reformatted, which is repo-wide drift orthogonal to CI gating; filed as **CU-021** (Category A, ~30 min) and the `format --check` step gets added in that follow-on. All other steps verified locally before landing: `ruff check src/` clean, `mypy --strict` clean (53 files), `lint-imports` reports 5 contracts kept, `pytest -q` 2798/2798 passing.

...
**Symptom** (post-slice-3, 2026-04-25): `pytest -m level0` collects 1307/2798; `pytest -m level1` collects 822/2798; `pytest -m level2` collects 11/2798; `pytest -m golden` collects 10/2798. The remaining 648 tests run under no level marker (slice 4 territory: `io/cli/tests/` + top-level `tests/`) and so cannot be gated by CI per §3.

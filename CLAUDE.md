...
**Consistency invariant**: Both paths originate from the same pupil. The FFT of the convolved `EffectivePSF` must agree with the MTF product; `performance/consistency_check.py` runs on every chain execution **in which the spatial (PSF/MTF) path is computed** with a default absolute tolerance of 2e-2 (worst measured full-chain discretization residual is ~1e-2 after the CU-003 area-integrated pixel kernel; the tolerance carries ~2× margin — see CU-003/CU-045), compares the paths **at and below detector Nyquist only** (CU-345, owner-ratified 2026-09-07: the old half-PSF-grid reach probed pixel-sinc sidelobes at 4–8× Nyquist, where the residual is pure discretization; every consumer metric and any genuinely missing term lives at or below Nyquist), and logs a warning on failure. A failure means a degradation was added to one path but not the other. The spatial path (and hence this check) is skipped only when the analyst deselects the entire Spatial-MTF metric group *and* no enabled metric needs a spatial input — there is then no spatial computation to check (Gap 96 metric selection; owner-ratified 2026-07-18).
...
### 21. Every Finding Is Recorded — a CU Only If It Earns One
When work uncovers a latent issue orthogonal to the current task — placeholder implementation, suppressed warning, dead helper, schema mismatch, doc claim that doesn't match code, golden-result tolerance bumped, hardcoded value that should be a parameter — it MUST be recorded **before the current PR merges**. No silently-deferred debt; no "I'll log it later". Recording is two-tier (owner-ratified 2026-07-31):

**Tier 1 — CU entry.** A finding becomes a CU in `docs/tracking/Cleanup_Backlog.md` only if it passes at least one of four intake tests:
1. **Results-affecting** — it changes (or a fix would change) a computed number;
2. **Owner-gated** — it needs a ruling from the project owner;
3. **Blocking** — other tracked work cannot proceed past it;
4. **Workflow-visible** — a defect an operator actually hits in a scenario or GUI workflow that is run.
...
**Tier 2 — Findings Log line.** Everything below that bar — dead helpers, stale docstrings, cosmetic warnings, test slowness, doc-guidance gaps, gate-scope quibbles — is appended as **one line** to `docs/tracking/Findings_Log.md`: date, `path:line`, one-sentence symptom, origin. No ID, no reservation push, no closure protocol. A log line has exactly three exits: **promoted** to a CU when someone decides to work it (strike the line in the promoting commit), **struck** in the same PR that fixes it in passing, or **expired** — a quarterly sweep deletes lines nobody promoted, which is legitimate because the intake test already ruled them non-critical. When unsure of the tier, ask: "would the owner schedule work for this?" If not, it is a log line.

**Family CUs.** Coupled findings in one analysis area (e.g. one physics workstream) are tracked as **one** CU with a checklist body, not one ID per fragment. Checklist items do not get their own IDs; a later finding that belongs to an existing family is added to its checklist (or folded into it per Rule 22), not minted fresh.

### 22. CU Closure Is a Recorded, Verifiable Event
A CU is closed only by moving its entry into the **Resolved** section of `docs/tracking/Cleanup_Backlog.md` with: resolution date, a closure record in the heading, and a one-line resolution summary. Phantom closure (deleting an entry, marking ✓ without a closure record, or moving to Resolved without one) is forbidden. Closure records come in two kinds (owner-ratified 2026-07-31):

- **Code-backed** (`RESOLVED` / `CLOSED`): the commit that lands the fix ALSO moves the registry entry and carries a `CU-Closes: NNN` trailer line in its commit message (comma-separate multiple numbers). The heading writes `(commit trailer)`; `scripts/check_org_rules.py` verifies a trailer commit exists in `main`'s ancestry. One commit closes the CU — fix, registry move, CHANGELOG, and lock-step docs together; there is no post-merge SHA-stamp commit. The legacy explicit-SHA form `` (commit `sha`) `` remains valid — mandatory for pre-2026-07-31 entries (frozen as-is) and available when a closure must cite a specific foreign commit.
- **Ruling-backed** (no code): `DECLINED` — will not do, one line of rationale. `ACCEPTED` — the owner ratifies living with the behavior; the heading's `(no commit — limitation: <one line>)` clause doubles as the model's known-limitation record. `FOLDED` — the finding becomes a checklist item on a family head (`(no commit — folded into CU-NNN)`); the folded body stays in Resolved for reference. `DEMOTED` — a Rule 21 intake re-test rules it below CU grade (`(no commit — demoted to Findings Log)`); its one-liner moves to the log. `SUPERSEDED` as before.

Stage-deferral rule: if a CU is intentionally deferred behind unrelated stage work, the entry MUST record the gating stage(s) and a re-audit date. When a gating stage lands, the next PR touching that area re-audits the CU and either closes it or refreshes the deferral record (new gating stage + new re-audit date). A CU may not be silently carried across multiple stage landings without re-audit. For a family CU, deferral and re-audit apply to the family as one unit.

### 23. Every Artifact Has One Defined Home — and a Conforming Name
`docs/OPERATING_MODEL.md` §1 defines the closed folder taxonomy (architecture / adr / guides / theory / validation / tracking / plans / reports / archive) and §5 defines the binding naming and format conventions for every non-source file (including §5.4: new documents are LaTeX-convertible Markdown — math as Pandoc-compatible `$...$`/`$$...$$`, no raw HTML in manual-class docs; typeset output is generated, never a hand-maintained `.tex` fork). Placement, naming, and format are review-blocking: no project-management markdown inside a Python package, nothing new at `docs/` top level, no status or version words in filenames. When a placement question isn't answered by the Operating Model, the answer is added to it in the same PR.
...
- **Process-machinery moratorium (owner-ratified 2026-07-31).** Do not extend or refine the gate battery, the registry checkers, closure-format tooling, or the tracking-file formats unless a real defect escaped to `main` through the specific hole being closed. Closure-metadata archaeology on entries closed before 2026-07-31 is permanently out of scope — the grandfather lists are frozen by design, not debt.
...
These rules keep concurrent work clean. Most are process rules enforced by
convention; the one hard, automated gate is the test battery that every merge to
`main` must pass (below) — that gate, not owner sign-off, is what protects `main`.
...
- **One agent, one workspace. Never two agents in the same working directory at once.** This is the load-bearing rule — it is what prevents files appearing under you and registry files changing mid-edit. If work genuinely overlaps in time, isolate with a **git worktree** per agent (`git worktree add ../ssr-<task> -b <task-branch>`), each on its own branch sharing one repo/history.
  - **Editable-install caveat (worktrees):** `pip install -e .` writes a `.pth` that pins `import radiant` to whichever checkout was installed (normally the **main** tree). `pytest` handles this itself — `pythonpath = ["src"]` in `pyproject.toml` (rootdir-relative) makes a test run inside a worktree import *that worktree's* `src/`. But plain `python` (scenario runners, ad-hoc scripts) — **and `lint-imports`, which imports `radiant` by name and so silently grades the *main* tree when run bare in a worktree (a gui→calibration test import reached `main` this way on 2026-09-20; found and fixed by the CU-377 batch)** — still resolves `radiant` from the editable install, so to exercise a worktree's **library** edits outside pytest, run with `PYTHONPATH=./src` (`PYTHONPATH=./src lint-imports` for the import gate) (or `pip install -e .` from the worktree, which re-points the shared `.pth` and thus affects every checkout — avoid when other worktrees are live). Editing scenario/doc/registry files in a worktree needs none of this; only `src/radiant/` edits are affected.
...
- **Delete every branch the instant it merges — local *and* its pushed `origin/<branch>`.** No branch outlives its merge. `git branch -d <b>` locally (`-D` if it was a throwaway that never needed review), and `git push origin --delete <b>` for the remote. Stale merged branches are the clutter that turned into a 20-branch cleanup; the rule is "merged ⇒ gone," not "merged ⇒ someday."
...
- **Registries are the collision hot-spots** — `docs/tracking/gaps.md`, `docs/tracking/Cleanup_Backlog.md`, `docs/tracking/Findings_Log.md`, `CHANGELOG.md`. Edits to them must be **small, append-only, and committed immediately** (a new CU/Gap goes in and gets committed on its own, before the surrounding task work), so concurrent edits land in different sections and merge trivially. Never leave a half-edited registry uncommitted across other work. Findings-Log lines carry no IDs, so they need no reservation push — append and commit with the task work.
- **Minting a CU/Gap number reserves it on `origin/main` — commit AND push the stub immediately.** `origin/main` is the reservation ledger: a number that exists only on an unpushed branch is invisible to every concurrent session, and "next available" read from a stale main is how two sessions mint the same IDs (it happened 2026-07-27: an overnight branch held CU-227–233 unpushed while a morning session minted the same range, crossing even the same two independently-discovered defects). Fetch before minting; push the registry stub (entry header + required fields is enough, body may follow on the branch) straight to `main` before continuing task work. `scripts/check_org_rules.py` enforces the invariant mechanically — duplicate leading CU/Gap heading IDs fail the gate battery, and a closure that copies instead of moves an entry (two headings, one number) fails the same check.
- **Never leave `main` checked out in a long-lived side worktree.** Merge from a checkout that is *on* `main` (or from a throwaway worktree that you `git worktree remove` immediately after). `main` parked in a scratch worktree locks the branch, stranding the primary checkout on a detached HEAD — exactly the confusion a persistent `ssr-merge2` caused. After any merge sprint, the resting state is a single checkout sitting on `main`, in sync with `origin/main`, with no extra worktrees.
- **Agents merge and push to `main` autonomously — no owner sign-off — once the full gate battery passes** (owner-ratified 2026-07-25; supersedes the prior "explicit owner approval" gate). The guardrail is the tests, not the owner: before a branch merges to `main`, **all** of `pytest -q` (fast; `testpaths = ["src", "tests"]`) + `pytest scripts/` + the touched golden tests, `mypy --strict src/radiant/core src/radiant/api`, `ruff check src/ tests/ scripts/ dev_tools/ scenarios/ examples/`, `ruff format --check src/ tests/ scripts/ dev_tools/ scenarios/ examples/` (CU-252 widened the format check, which was `src/`-only in CI and absent from this list, so 44 files drifted unseen; CU-270 widened the lint check to the same four trees, where 15 further violations had accumulated unseen; the October-sweep ruling, owner-ratified 2026-09-12, added `scenarios/` and `examples/` — examples ship in the wheel since Gap 126, and 1253 findings including Py3.12-only f-strings had accumulated in scenarios/; the scenario style residue is grandfathered in `pyproject.toml` per-file-ignores, retired by the chartered scenario sweep. Neither new tree joins pytest: scenario runners are verified by the §5.3 walkthrough sweep, not the merge battery), `lint-imports`, `python scripts/check_org_rules.py`, and `python scripts/gen_param_reference.py --check` must pass. **`scripts/` is in scope deliberately** (CU-272, decided 2026-07-29): it is outside `testpaths`, so bare `pytest` never reached it and `scripts/synth_modtran/tests/` sat red on every clean tree — a real regression in the synthetic-MODTRAN tooling that feeds the interpolated libraries would have been indistinguishable from that environmental red. It runs in under a second. `dev_tools/` is **not** in the local battery: its only suite is `geometry_gui_v2`, which needs a display and is run under Xvfb by CI. Both trees are already covered by the ruff lint and format checks above. A change that cannot pass the gates does not reach `main`, and there is no manual override. Push the merged `main` immediately (`origin/main` is the shared source of truth); keep the branch current with small merges rather than one large end-of-life reconciliation.
  - **Never pipe a gate's pytest through another command** (`pytest -q | tail -1` and kin): the pipeline's exit status is the last command's, so a red suite reads green and the summary line can be displaced by teardown noise. It took `main` red once (2026-08-03, `474cd0c8`) and nearly passed an unverified suite again 2026-09-08. Redirect to a file and check pytest's own exit status (`pytest -q > out.txt 2>&1; echo $?`), or `set -o pipefail`.
  - **`pytest scripts/` runs bare** as written above. It used to abort at collection with `ModuleNotFoundError: No module named 'scripts'` — only `python -m pytest scripts/` worked, because `python -m` puts the CWD on `sys.path` and bare `pytest` does not. CU-280 fixed the invocation rather than the documentation, by adding the repo root to pytest's rootdir-relative `pythonpath` in `pyproject.toml`; the `scripts.synth_modtran...` rooting the two test files use is the same rooting every other consumer of that package uses, so rerooting the tests alone would have forked the convention. CI runs the same bare form.
  - **A GUI-only diff runs a scoped battery** (owner-ratified 2026-08-29): when a branch's source diff is confined to `src/radiant/gui/**` (docs, registries, and `CHANGELOG.md` may ride along), the merge gate is the **full GUI suite** (`pytest src/radiant/gui/tests -q`) + `pytest scripts/` + every static gate (ruff lint and format, mypy, `lint-imports`, `check_org_rules`, `gen_param_reference --check`) — the physics/golden/integration portion of `pytest -q` is skipped. This narrowing is sound where touched-file heuristics generally are not because the import boundary is machine-enforced: nothing outside `gui/` imports `gui/` (`import-linter`), and the schema lives in stage `_schema.py` files outside `gui/`, so a gui-confined diff cannot move a physics result, golden baseline, or schema surface. Any diff touching `src/` outside `gui/` — one `_schema.py` line is enough — pays the full battery; the scoping never runs in that direction (CU-262/CU-309).
  - **A change touching any `_schema.py` runs the full GUI suite** (`src/radiant/gui/tests/`, ~7 minutes), regardless of which files it edited. `pytest -q` covers it — `testpaths` includes `src` — so this is a rule against *narrowing* the battery, not an addition to it. The tempting shortcut is to select suites by "which files did I touch"; that predictor is unsound, because GUI tests assert against the **schema** through `Sensor.parameter_defs()`. CU-262 added one `geometry.*` parameter, touched no GUI file, and took `main` red on a GUI test (CU-309). Schema surface changed ⇒ GUI suite runs.
...
| Discover a latent issue in passing without recording it (CU or Findings-Log line) before PR merge | Violates Rule 21 — produces silent debt |
| Mint a full CU for a finding that fails all four Rule-21 intake tests | Violates Rule 21 — ceremony spent on trivia; it's a Findings-Log line |
| Mark a CU resolved without a closure record (`CU-Closes` trailer or SHA) and resolution date | Violates Rule 22 — phantom closure |
...
# Tooling under scripts/ (CU-272 — outside pytest's testpaths, so bare `pytest`
# misses it; deck-dependent tests skip when the generated tape7 set is absent).
# CU-280 put the repo root on pytest's `pythonpath`, so this bare form now
# collects; before that only `python -m pytest scripts/` resolved `scripts.*`:
pytest scripts/ -q
...
# Lint (CU-089 widened the gate to tests/; CU-270 to scripts/ and dev_tools/;
# the 2026-09-12 October-sweep ruling to scenarios/ and examples/ — linter and
# formatter govern the same six trees; scenario style residue is grandfathered
# in pyproject per-file-ignores):
ruff check src/ tests/ scripts/ dev_tools/ scenarios/ examples/

# Formatter (must be in the gate battery — this is the check that was missing):

...
**Scheduled-state seam (CU-289).** `RADIANTMainWindow.evaluation_scheduled` is the
read-only view of that timer: `True` while a debounced re-evaluation is pending and has
not started, `False` otherwise. It exists because the transaction tests (configuration
manager, configured parameters, selector undo) owe the contract "this edit really does
trigger a re-evaluation" but read only synchronously-applied state afterwards — so they
assert the scheduled state instead of awaiting a full evaluate-all pass whose result they
discard. The assert still **fails** if an action stops scheduling, and it removes the
wall-clock dependence that made the merge gate load-sensitive (CU-314). Tests that read
`last_run` / `last_result` / a rendered readout still await `evaluationFinished`, as does
any path that starts the worker directly (`_evaluate_now`, File → Open) rather than
...
*production* use of the full mechanism — four tabs (**Inputs | Transmission | MTF |
PSF + Pupil**) carrying the richest per-stage content (editable inputs + FINAL-regime
outputs, the one home for how τ_opt is defined, the MTF table + overlay, and the PSF +
FP-2 complex-pupil maps). The hook is the data seam (`StageComposition.subviews` of
`StageSubView`, `radiant.gui.stage_views`); a stage with zero or one sub-view falls back to
the single flat pane.

*Optics **Transmission** tab (owner-ratified 2026-09-09/10).* The Optics strip shipped with
five tabs, of which **Elements** and **Throughput** were two halves of one question — *how
is optical transmission defined, and what does that produce?* Splitting them let an analyst
edit the scalar `optics.transmission_scalar` on **Inputs** while an attached element train
silently overrode it, with nothing on that screen saying so. The two are now one
**Transmission** tab, top to bottom:

1. a **mode selector** (`TransmissionModeSelector`, segmented control) — *Scalar throughput*
   | *Element train*;
2. a **mode banner** naming the definition in force and what it makes irrelevant
...
5. the **cold stop & effective pupil strip** (`EffectivePupilReadout`, Gap 128) — the two
   ...
   `stage_outputs["optics"]` (`—` before the first evaluation). Specified by the 7.2 and
   10.1 scenario `gui_workflow.md` files.

**Mode semantics.** The mode is not a parameter — it is the *shape of the document*, so it
is read from the document and never invented: a config carrying an `optical_elements` list
opens in element mode, anything else in scalar mode. Switching is **non-destructive
in-session** (leaving element mode detaches the document through one
`set_optical_elements(None)` while the rows stay in the table, inactive, so the operator can
A/B the two definitions; switching back re-commits them) but **single-mode on disk**: saving
writes only the active mode, so a file with an element list means element mode, period, and
no hidden inactive state ever reaches a config. While rows are held the banner carries the
warn register, and the save confirmation adds a non-modal status-bar note naming what was
...
| **Atmosphere** | Model + propagation inputs (model selector, per-model knobs, turbulence r₀) | **[SHIPPED — GUI Capability Expansion plan GS-2]** `AtmosphereInputsForm` — the `atmosphere.model` selector (`simple`/`exo`/`tabulated`/`modtran`/`interpolated`) with **only the active backend's group visible** (simple: profile/aerosol/visibility/PWV; modtran: tape7 + sun-path tape7 + profile/aerosol/H₂O/O₃ scaling; tabulated: the three file params; interpolated: run-matrix dir/axes/method; exo: a themed note) + `atmosphere.r0_m`, as shared `FieldRow`s, one `sensor.set` per edit. **Interpolated-family selection (CU-239, SHIPPED):** the free-text `interpolation_axes` row is gone; the *Interpolated run matrix* group leads with `AtmosphereFamilyPicker` (own file, Rule 19), a list built from `radiant.api.shipped_atmosphere_families()` — one row per bundled family labelled with its name and rendered profile, the selected row's plain-language coverage line beneath (**units always explicit**: km, degrees), and *Custom axes… (advanced)* as the last entry, which opens the same `ParameterEditorDialog` on `atmosphere.interpolation_axes` for a hand-built run matrix (the escape hatch; an unrecognized configured value selects it rather than being hidden). Choosing a row writes `interpolation_axes` — and `interpolated_data_dir` for a family no axes key can reach — as **derived values**, emitted as one compound edit so the two reverse in a single undo step (CU-141). `Sensor.suggested_atmosphere_family()` supplies the scene's recommendation: that row is marked *(recommended for this scene)*, and when what is configured cannot serve the scene the recommendation is **pre-selected as a proposal** — shown and costed but not written, with an explicit *Use this family* button — because adopting a family can change the atmosphere profile. **Pre-validated recommendation + one advisory (CU-322, SHIPPED):** that recommendation is now checked against the *complete* query before it is offered (direction, axes, LOS zenith, target ceiling, rendered lower endpoint), so the picker never proposes a family the chain would then refuse, and it can propose an `explicit_dir_only` row by name. "Cannot serve the scene" is decided by **both** `Sensor.validate_atmosphere_coverage()` (do the axes reach a family at all) and `Sensor.atmosphere_family_gap()` on the configured family (can *that* family serve this query) — the second was the missing half, and is why an off-vertical up-looking scene with a valid axes string used to reach Evaluate before failing. The highlighted row's line appends `Does not serve this scene: <gap>` (units explicit) when the gap is non-`None`. When **no** bundled family serves the scene, `_refresh_coverage_advisory` puts a single `Sensor.atmosphere_family_suggestion().advisory_error()` in the Messages rail naming the closest miss, and that takes precedence over the axes-coverage refusal — which would otherwise send the operator to set an axes string that lands on the next refusal (the error wall CU-322 measured). **Evaluate-time coverage refusals are advisories, not modals:** `_on_eval_failed` routes any error `radiant.api.is_atmosphere_coverage_refusal()` recognises to the Messages rail and the status bar and shows **no** `ActionableErrorDialog` — the *Parameter Rejected / Cannot set "evaluate"* modal is for inputs the framework rejected, and a coverage refusal is a legal scene the bundled library has no measured column for. Routing is by exception type and structured `context`, never message text; genuine parameter rejections keep the modal. Whenever the highlighted family's profile differs from an explicitly-set `atmosphere.standard_atmosphere`, `Sensor.atmosphere_profile_change_warning()`'s sentence shows beside it. `midlat_summer_boost_ladder` is offered **by name** (ex-CU-296): its 24 runs share the 2-axis ladders' key and no axes string reaches them, so the picker writes the bundled directory instead. **Coverage check on edit:** `RADIANTMainWindow._refresh_coverage_advisory` runs `Sensor.validate_atmosphere_coverage()` after every parameter edit and puts a refusal in the Messages rail immediately — not ~1 s later as a failed evaluation — and clears it again when the config becomes coverable. It owns only the item it placed (Rule 17: it never clears an error another surface reported) and skips an unresolvable mid-edit config. File paths are string schema fields (the plan's file-picker fallback; the ADR-0009 D5 preview dialog is the follow-on). Editing re-evaluates and every plot below refreshes |
...
| **Optics** | Element-train editor (per-element R/T/temperature, ε derived) | **[SHIPPED — GUI Capability Expansion plan GS-4; relocated into the **Transmission** tab 2026-09-09/10]** `OpticalElementEditor`, mounted by `TransmissionPanel` as the element-train mode's editor — the ADR-0009 D2 declarative-document editor: table rows ⇌ the `optical_elements:` entry dicts (R/T cells take a scalar, a spectral-CSV path — typed, or picked with the *CSV file…* button's file dialog — or an inline λ-table via *Spectrum…*). **Entry-faithful (CU-344):** a row carries its source entry and a commit is that entry with only the cells the table owns overlaid, so cells for keys an entry does not carry are **blank and commit nothing** (the io parser's own defaults keep applying; typing into a blank cell writes the key, clearing one removes it), every key the table has no column for — a refractive row's `reflectance`, a cavity row's per-surface fields — rides through untouched, and a combo never rewrites the source's spelling (`kind: FILTER` stays `FILTER`); entries therefore round-trip byte-faithfully and an edit to one row cannot change another row's physics. A transfer-mode flip is the one deliberate deletion (the old mode's value key, and `kind` when flipping to REFLECTIVE). **commit-on-edit (owner-ratified 2026-09-03 in live review, retiring the ADR-0009 D4 *Apply train* button):** every completed edit — a cell edit, a transfer/kind combo change, a *CSV file…* pick, a *Spectrum…* OK, Add, Remove, reorder — writes the document as it is made through one `Sensor.set_optical_elements` call (io-parser validation, Kirchhoff checks), exactly as any parameter field commits, and the debounced re-evaluation follows; the batched model it replaces held drafts that *looked* committed (the owner edited 0.97 → 0.5, saw no SNR change, and lost the edit on navigation). A row the parser rejects becomes a **pending draft**: it stays in the table, the parser's actionable message is shown inline under the table in the warn register (`stagePlotMessage[state="pending"]`) with the note that it is not committed and that leaving the tab discards it, **no modal is opened**, nothing is stored — and the next edit that validates commits the whole train and clears the state. A rejection that is not row validity (a `ConfigSetError` on a configured-row write) still shows the actionable dialog and reverts the table to the stored document; the ε column is **derived read-only** (Rule 5 — 1−R mirrors, cavity/zero refractive, from `preview_optical_elements`); the attached train persists through `Sensor.save`/`load` (ADR-0009 D4) and the optics stage runs full-prescription on the next evaluation. **Coating detail (Gap 116):** selecting a table row draws that element's R/T/ε below the table via one `radiant.api.plot_coating_detail` call — native source grid (full stored extent, not the run band), one autoscaled panel per quantity, evaluation band shaded; the table's own entries pass through `entries=` so a row previews as it is typed (a pending draft's inline message owns the pane while it is held), and a row the io parser rejects shows its actionable message in place of the figure. The pane follows the app's dark/light plot theme (`set_dark`, CU-139 seam). **Configured element rows (Gap 103 v1.1, owner-ratified 2026-09-02 in live review, superseding the scope-control/override model of the same morning):** an element **row** configures exactly like a parameter. In a **multi-member study** the tab renders the displayed configuration's train (`ConfigurationSet.effective_optical_elements(active)` — the shared skeleton with each configured row resolved to that configuration's entry), so the coating-detail pane inspects that configuration's element, and the table lands with row 0 selected so every selection-driven affordance starts live. **A selection-tracking *Configure* button** (label flips between the two actions with the selected row — the visible entrance, added 2026-09-03 after the right-click-only affordance proved undiscoverable in live review) **and right-clicking a row** (`row_menu(row)`) offer the two actions the parameter surfaces offer, worded from the same `configure_menu` module: *Configure across configurations…* → one `ConfigurationSet.configure_element(row)` (seeds every configuration with the row's current shared entry, so the promotion changes no result, and moves it out of the shared document — single store), and *Un-configure row (keep <first>'s entry)…* → one `unconfigure_element(row)` behind a confirmation naming the entry that survives (D-6 keep-first) and the ones discarded. An added row is in the document at once (the Add templates are complete valid entries, so the append commits), so it is configurable immediately — the former unapplied-row limbo state is retired. A configured row carries the **red "C"** after its name, painted by `EditableConfiguredNameDelegate` (the one `configured_badge_icon` glyph, `err` token) so the Name cell keeps its inline editor — row identity is **positional** and the entry's `name` configures with the row. **D-8 inline edit:** each commit splits the table by the document's own single store — the shared rows go, in table order, to one `set_optical_elements` on the set's **base**, and each configured row's entry to one `set_element_for(row, displayed, entry)`, so every other configuration's entry stays verbatim. Every row is validated through the io parser *before* anything is written, so a rejected train stores nothing; an invalid configured row is re-offered to `set_element_for` purely to raise the API's own message, which **names the configuration** in the inline pending note. The train's **structure is shared** — same row count and order in every configuration — so Add appends a shared row for all of them and Remove drops one from all of them (with the same per-configuration confirmation when the row is configured). An edit that *shifts* a configured row (removing a row above it, reordering across it) goes through the position-preserving `ConfigurationSet.remove_element` / `move_element` operations (CU-357), which renumber the configured rows with every configuration's entries intact — the pre-CU-357 disabled-button refusals ("un-configure it first") are retired. **Element undo (CU-357):** every successful commit — a cell edit, a structure change, a configure-across, an un-configure, a mode detach — also emits `trainCommitted` with whole-train before/after `ElementTrainState` snapshots (both stores as one unit), which the window records as one `ElementTrainCommand` (`widgets/element_train_command.py`); undo/redo restore the pair of stores together through `ConfigurationSet.restore_element_state` (or `Sensor.set_optical_elements` in a plain session), re-render the table, and re-evaluate — closing the documented Phase-9 non-scalar-edit undo limitation. A single-configuration session shows no study note, no row menu, and no badges, and behaves byte-for-byte as the paragraph above. The tab takes the session's `ConfigurationScope` through the one `bind_configuration_scope` fan-out, re-reads on a document change and on every selector switch (`bind_sensor`) but never on a configured-value change (an in-progress table edit is not clobbered), and the window re-materializes the displayed sensor after every element commit — an edit, a configure, or an un-configure, all of them emit `elementsApplied` — so a study's displayed train is never the pre-commit one; that re-bind arrives *during* the commit and is skipped by the tab (the table is the document just written), which is what keeps the host's re-render from fighting the edit that caused it, while the selected row survives any re-render |
| **Optics** | MTF | **[SHIPPED — GUI plan Phase PS-2]** `result.plot.mtf()` overlay + the per-term MTF@Nyquist table (`MtfPanel`) + the `mtf_budget()` bar, in the Optics **MTF** tab. The overlay draws the **SYSTEM** product (ink, heavier, from `stage_outputs["performance"]["mtf_budget"]`) above the contributors and bounds its x-axis to 2 × max(Nyquist, optics cutoff) cycles/mrad — both arrive through the one `result.plot.mtf()` call, so the panel needed no change (2026-09-09 Windows-deployment feedback; see RADIANT_Scripting_API §5.1) |
...
| **Detector** | FPA part library (named presets) | **[SHIPPED — Gap 119 plan Phase 3]** `FPAPartSelector` (one compact row) + `FPAPartPickerDialog` (table: Kind [FPA vs bare-ROIC, Gap 121] / Class / Band / basis census, details pane) — pick a real part from the 21 shipped presets and apply it in one `sensor.apply_fpa` call; the status row summarizes applied-vs-kept (presets seed, explicit wins) with a per-parameter Details dialog; Open datasheet/paper opens the committed PDF (`docs/validation/fpa_datasheets/`) in a repo checkout, else the citation URL/DOI. Part metadata via `radiant.api.fpa_preset.available_fpa_parts` (GUI never imports `radiant.data`). Sits above the Inputs card in the Detector **Inputs** tab; a successful apply re-enters the debounced re-evaluation path |
| **Detector** | Editable detector inputs + outputs | **[SHIPPED — GUI plan Phase PS-3; expanded to the full schema by GUI Capability Expansion plan GS-3]** `DetectorInputsForm` — **every** `detector.*` ParameterDef as shared `FieldRow`s (a manifest-equals-schema test enforces completeness), grouped: pixel geometry & temperature; pixel sampling phase (Gap 129 — `pixel_phase_mode` enum row edited through the shared editor's combo + `pixel_phase_x/y`; the Detector + PSF tab's `psf_pixel_grid` overlay shifts to the chain's resolved phase and names the mode in its title); QE (scalar / `qe_table_path` CSV import / temperature coefficients); dark current & glow (Arrhenius); 1/f noise (K + band); G-R & Johnson; FPN (PRNU/DSNU/clutter σ/`noise_regime`); persistence (fraction/τ/prior signal); IPC & diffusion. One `sensor.set` per edit (the edit+reject discipline), beside the scalar `OutputsReadout` (`signal_e`, `dark_e`, …, units from `api.stage_output_units`), in the Detector **Inputs** tab; editing re-evaluates and every tab refreshes (edit-and-watch) |
...
| **Calibration** | Scheme selector + contextual groups + residual/bias readout | **[SHIPPED — Gap 120 plan Phase 3; live-review approved 2026-09-07 after three passes: blank-config edit prompt + pin reset, and pre-result editable stage screens]** `CalibrationInputsForm` — the `calibration.scheme` selector leads (the card that steers what the rest means; the Geometry scene-class / Gap 117 architecture-selector rationale), then scheme-contextual groups (cal points, NUC nonlinearity under `two_point` only, drift-since-cal, cal-source and gain uncertainty — the bias-budget knobs), all shared `FieldRow`s (one `sensor.set` per edit, clone-validate commit, session display units — hours for time-since-cal). Unset cal temperatures render as words ("unset — required"), never "0 K" (the Gap 117 second-pass lesson applied from birth). Beside it: the `OutputsReadout` on `stage_outputs["calibration"]` (cal signals, per-term residuals, `sigma_calibration_e`, post-calibration `sigma_total_e`, `bias_total_frac`, the NEDT-equivalent calibration floor — units via `api.stage_output_units`) and the noise-budget plot where the residual terms appear once a scheme is active (edit-and-watch). A mid-switch config (scheme active, cal point unset) routes as an advisory beside the inputs via `api.calibration_state.is_calibration_config_incomplete` — only the calibration chip paints as the error site, never a modal per evaluation (the CU-322 pattern). Accuracy metrics render on the Performance cards — precision and accuracy side by side, never one number |

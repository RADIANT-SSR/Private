...
**Calibration range (Gap 22).** GIQE-5 is a fit; outside its published fit ranges (Harrington et al. 2015) the value is an extrapolation with reduced confidence, not an error. Checked ranges: GSD 3–80 cm (1.18–31.5 inch), RER 0.2–0.95, SNR 2–130 — both ends. When any input is out of range the formula value is still returned, `GIQEResult.extrapolated` is `True` with per-input strings in `GIQEResult.warnings`, and `result.metrics["niirs_extrapolated"]` is 1.0 (else 0.0). The IIRS (MWIR/LWIR) dispatch inherits the same checks. The out-of-range condition is carried as **structured status only** — as of CU-166 the chain does **not** emit a `UserWarning` (nor a `logger.warning`) for it: a metric falling outside its own calibration band is a property of the configuration, not a per-evaluate event, so re-announcing it every evaluate floods sweeps and the GUI console (owner bar 2026-07-18: a valid, nominally-operating scenario evaluates warning-free). Consumers render the caveat once from `extrapolated` / `niirs_extrapolated`. (Deferred, gated on the Gap 96 metric-selection decision: up-front metric-applicability gating so `PerformanceStage` returns NIIRS as N/A when clearly out of range, and the MWIR→IIRS-vs-GIQE-5 routing/label question.)

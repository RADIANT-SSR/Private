...
**Repo root is a closed list too:** `README.md`, `DEVELOPMENT.md`, `CHANGELOG.md`, `CLAUDE.md`, `pyproject.toml`, `mkdocs.yml`, `.gitignore`, `.gitattributes`, `.pre-commit-config.yaml` (plus `LICENSE` if one is added). Root-level `README`/`DEVELOPMENT`/`CHANGELOG` follow ecosystem convention — they are the files a newcomer looks for before knowing the taxonomy exists. `CHANGELOG.md` (added 2026-07-07) is the Rule-29 record of behavior-affecting changes: living, append-at-top, Keep-a-Changelog format; it complements the tracking registries, which remain the work board. Any other root file needs a row added here first.

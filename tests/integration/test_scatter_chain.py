...
@pytest.mark.level2
class TestScatterKernelGridCap:
    """Regression: kernel size capped by an even PSF grid must stay odd.

    With a fine PSF sample spacing (VNIR band, small pixels) the 6σ halo
    span exceeds the PSF grid, so the kernel size is capped to the grid
    size — which is even (256). The stage must cap to the largest odd
    size instead of passing the even grid size to scatter_kernel_2d
    (found by scenario 7.3 refresh, 2026-07-07).
    """

    def test_capped_kernel_runs(self) -> None:
        wl = np.linspace(0.5, 0.9, 200)
        session = RadiantSession(wavelength_um=wl)
        p = session.default_params()
        p.set("source.target.temperature", 3000.0)
        p.set("source.target.emissivity", 0.95)
        p.set("optics.aperture_diameter_m", 0.20)
        p.set("optics.focal_length_m", 0.60)
        p.set("optics.transmission_scalar", 0.80)
        # Halo σ (default 100 µm) ≫ PSF grid extent at VNIR spacing → cap.
        p.set("optics.surface_roughness_nm", 10.0)
        ...
        p.set("geometry.sensor_altitude_m", 8000.0)
        p.set("atmosphere.standard_atmosphere", "midlat_summer")
        p.set("spectral_integration.filter_min_um", 0.5)
        p.set("spectral_integration.filter_max_um", 0.9)
        p.set("spectral_integration.integration_time_s", 0.005)
        p.set("readout.read_noise_e_rms", 5.0)
        p.set("readout.gain_e_per_dn", 32.0)
        p.set("readout.adc_bits", 16)
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", UserWarning)
            result = session.run(p)  # raised ValueError (even npix) pre-fix

# GUI scripting-window script — Scenario 3.4: Off-nadir agility — nadir baseline config
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/3.4_off_nadir_agility.gui.yaml
...
#
# NOTE: Runner's module-level base_config (nadir); the sweep varies path zenith. NIIRS-vs-off-nadir is the headline trade, so allow_extrapolated=true opts the baseline into the GIQE-5 rating past its SNR-calibration envelope (CU-166/CU-170).
#
...
# (headless smoke test): python scripts/gui_console_3.4_off_nadir_agility.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "3.4_off_nadir_agility.gui.yaml"
    )
...
print("=== Scenario 3.4: Off-nadir agility — nadir baseline config ===")
...
def _sweep_and_plot():
    center = 0.35  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 3.4: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

# GUI scripting-window script — Scenario 5.3: Chromatic PSF — monochromatic baseline (single wavelength)
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/5.3_mono_vs_poly_psf.gui.yaml
...
#
# NOTE: Single-wavelength (mono) PSF baseline; poly variant raises psf_n_wavelengths. GUI baseline shortens integration to 1 ms (well ~52%) and uses a well-matched gain (32 e-/DN) so the 14-bit ADC no longer clips at the runner's 2 ms point (CU-170).
#
...
# (headless smoke test): python scripts/gui_console_5.3_mono_vs_poly_psf.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "5.3_mono_vs_poly_psf.gui.yaml"
    )
...
print("=== Scenario 5.3: Chromatic PSF — monochromatic baseline (single wavelength) ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 5.3: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

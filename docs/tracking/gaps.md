...
| **Status** | CLOSED 2026-09-03 (Configuration_Set_Expansion_Plan Phase 2, branch `cfgset/phase2-elements`, owner-approved after two days of live review). **Delivered as configured element rows** (plan §3a-bis, superseding both the D-7 whole-document idea and a same-day replace-by-name override design): a row of the shared `optical_elements` document configures like a parameter — dense one-complete-entry-per-member (name included; positional row identity), YAML in-place `- configured: {member: entry, …}` form, scripting API `configure_element` / `set_element_for` / `element_for` / `unconfigure_element(keep=)`, GUI red-C rows with commit-on-edit (D-8 per-member editing). ADR-0010 D-7 carries the dated supersession note. Prior deferral history: DEFERRED 2026-07-25 (D-7); re-audit 2026-08-29 fired the gating condition (scenario 9.4's nine OLI-2 bands, composite butcher-block workaround, pan band excluded); PLANNED 2026-09-01. The 9.4 all-bands showcase is plan Phase 3. |

# GUI scripting-window script — Scenario 2.1: Detector shootout — InSb baseline
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/2.1_insb_vs_hgcdte_noise_budget.gui.yaml
...
#
# NOTE: InSb option; the HgCdTe variant is the other build_config branch.
#
...
# (headless smoke test): python scripts/gui_console_2.1_insb_vs_hgcdte_noise_budget.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "2.1_insb_vs_hgcdte_noise_budget.gui.yaml"
    )
...
print("=== Scenario 2.1: Detector shootout — InSb baseline ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 2.1: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

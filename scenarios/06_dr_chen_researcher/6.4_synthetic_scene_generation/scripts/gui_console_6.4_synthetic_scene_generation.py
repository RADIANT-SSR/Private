# GUI scripting-window script — Scenario 6.4: Synthetic scene — first target pixel baseline
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/6.4_synthetic_scene_generation.gui.yaml
...
#
# NOTE: First scene target (temperature, emissivity, range->altitude).
#
...
# (headless smoke test): python scripts/gui_console_6.4_synthetic_scene_generation.py
...
print("=== Scenario 6.4: Synthetic scene — first target pixel baseline ===")
...
def _sweep_and_plot():
    center = 0.05  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 6.4: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

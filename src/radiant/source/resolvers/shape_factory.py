...
``source.target.shape`` onto the concrete shape class and wires the
...
def _orientation_rad(params: ParameterSet) -> tuple[float, float, float]:
    ...
    return (
        float(params.get("source.target.shape_yaw_rad")),
        float(params.get("source.target.shape_pitch_rad")),
        float(params.get("source.target.shape_roll_rad")),
    )
...
def build_shape(params: ParameterSet) -> TargetShape | None:
    ...
    Reads the resolved ``source.target.shape*`` scalars from ``params``
    and dispatches on the ``source.target.shape`` enum (see
    ...
    TargetShape | None
        ...
        back to ``source.target.projected_area_m2``).  Otherwise a
    ...
    if shape_name == "sphere":
        ...
        raise ParameterBoundsError(
            what=f"source.target.shape = {shape_name!r} is not recognised",

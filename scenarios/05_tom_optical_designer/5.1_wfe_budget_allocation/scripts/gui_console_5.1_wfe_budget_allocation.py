# GUI scripting-window script — Scenario 5.1: WFE budget — nominal base config
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/5.1_wfe_budget_allocation.gui.yaml
...
# (headless smoke test): python scripts/gui_console_5.1_wfe_budget_allocation.py
...
print("=== Scenario 5.1: WFE budget — nominal base config ===")
...
def _sweep_and_plot():
    center = 0.4  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 5.1: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

# GUI scripting-window script — Scenario 1.4: TDI pushbroom — baseline config (N_tdi=1 reference)
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/1.4_tdi_pushbroom_optimization.gui.yaml
...
#
# NOTE: Runner's module-level base_config (nominal); the N_tdi sweep varies from here. NIIRS is the scenario's headline trade metric, so allow_extrapolated=true opts the baseline into the GIQE-5 rating past its SNR-calibration envelope (CU-166/CU-170).
#
...
# (headless smoke test): python scripts/gui_console_1.4_tdi_pushbroom_optimization.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "1.4_tdi_pushbroom_optimization.gui.yaml"
    )
...
print("=== Scenario 1.4: TDI pushbroom — baseline config (N_tdi=1 reference) ===")
...
def _sweep_and_plot():
    center = 0.25  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 1.4: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

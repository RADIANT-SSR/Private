...
- **Supported per backend (Phase 2, 2026-07-26).** Ground-based and up-looking observers and equal-altitude/horizontal paths are **expressible, resolved, and served**. Phase 1 opened the geometry (the `h_sensor > h_target` gate is gone, $\theta_o$ spans the closed domain $[0, \pi]$, `LineOfSightGeometry` carries both endpoints, and the near-horizontal band is judged by the tangent-topology horizon guard); Phase 2 supplied the radiometry — `atmosphere/topology.py` dispatches on the derived LOS direction, and up/level paths are served by path-segment composition. **Which backend serves what:** `atmosphere.model = "simple"` serves every endo-atmospheric up-looking and level path (A1, A5, B2, and the per-altitude illumination test); **any** backend serves a wholly-vacuum path with both endpoints at or above `h_atm_top` (LEO→GEO); `interpolated` ships one up-looking family (`midlat_summer_uplooking_ladder`, vertical ground→0–20 km) reachable through `InterpolatedAtmosphere.uplooking_column_product`, and `modtran` gained `ITYPE=1` / `hrange_km` deck support — but neither is wired into the chain's up/level dispatch yet, so both still raise an actionable **capability** error naming `simple`. What remains scheduled: direction-aware turbulence and metric conditioning (Phase 3), GUI (Phase 4), scenario close-out (Phase 5). Wherever this document describes an unimplemented scene it says so explicitly.
...
| `air` | 1 km ≲ `h_sensor` ≲ 100 km | Down-looking (`h_sensor > h_target`): **supported today** (ratified, GF-6). Level and up-looking arms: **supported since Phase 2 (2026-07-26)** on `atmosphere.model = "simple"`; other backends raise a capability error naming it |
| `ground` | `h_sensor` ≲ 1 km | **Supported since Phase 2 (2026-07-26)** on `atmosphere.model = "simple"`. Phase 1 stopped the geometry core rejecting up-looking and equal-altitude paths; Phase 2 supplied the radiometry they need — the up-path observer leg (A1), the constant-altitude arm (A5), and the `SkyBackground` terminus (B2). Other backends raise a capability error naming `simple`. Ground-to-air (E2) and ground-to-space (E3) run end to end; E3's VIS sky background is provisional (decision 20) and its spatial performance still awaits the Gap 110 turbulence upgrade |
...
20. **Sky background (B2 / `SkyBackground`) is ratified, band-gated at first delivery.** MWIR/LWIR sky backgrounds ship in Phase 2; VIS/NIR sky computes but carries a "provisional — single-scatter underestimates daytime sky" `UserWarning` until MODTRAN-anchored. Extends decision 11. **DELIVERED 2026-07-26 (plan Phase 2)**: `core/descriptors.SkyBackground` selected by `core/los_termination.py`, radiance from `atmosphere/sky_radiance.py`, band gate in the same module (`SCATTERED_SKY_PROVISIONAL_MAX_UM = 3.0 µm`, warning raised only when the grid reaches below it *and* the sun is above the local horizon).

21. **Illumination becomes per-altitude.** The global $\theta_s < \pi/2$ bound is replaced by a shadow-height test — the target is sunlit iff its altitude exceeds the terminator shadow height for the given solar depression — enabling sunlit-target-over-dark-ground (GF-9). **DELIVERED 2026-07-26 (plan Phase 2)**: `geometry.solar_zenith_rad` and `AtmosphericGeometry.solar_zenith_rad` are bounded at $\pi$, and `atmosphere/solar_shadow.py` owns the test. Night scenes are still expressible the old way (`geometry.solar_illumination = "night"`, mode S0, θ_s = Δφ = None); nothing about a $\theta_s \le \pi/2$ scene moved. Caveat: the shadow test conditions the **up/level** arm's $\tau_{sun}$ only — a down-looking scene keeps the backend's own answer byte-for-byte, which for $\cos\theta_s \le 0$ is a vertical-column $\tau_{sun}$ rather than zero (nothing consumes it; assembly's clamped $\cos\theta_s$ zeroes the term either way).
...
- `A1` — **Up-path only** (sensor below target: column `h_sensor → h_tgt`, zenith taken at the lower endpoint). *Was "ground-based sensor; v2".* **Delivered 2026-07-26 (plan Phase 2)** on the simple backend: `atmosphere/topology.py` dispatches on the derived LOS direction and `atmosphere/uplooking_quantities.py` composes the segment.
...
- `A5` — **Horizontal constant-altitude arm** (equal or near-equal endpoint altitudes): Beer-Lambert at local density in the simple model, MODTRAN `ITYPE=1`. **Delivered 2026-07-26 (plan Phase 2)** on the simple backend (`atmosphere/level_arm.py`); MODTRAN `ITYPE=1` deck wiring landed alongside it, its library family is batch 2.
...
- `B2` — **Sky radiance along the LOS** (`SkyBackground`): the atmospheric column the ray traverses beyond the target before reaching space, for up-looking and near-horizontal paths. *Was "sky path looking up (v2)".* **Delivered 2026-07-26 (plan Phase 2)**, band-gated as ratified: MWIR/LWIR first-class, VIS/NIR computes with a provisional `UserWarning`. `core/descriptors.SkyBackground` carries no user parameters — the radiance is computed from the scene by `atmosphere/sky_radiance.py` (or `atmosphere/segment_grazing.py` for a near-tangent continuation past the 89.5° column ceiling) and passed into assembly.
- `B3` — GroundBackground
...
- **LOS direction** — down ($\theta_o < \pi/2$, sensor above the target's horizon plane), level ($h_{sensor} = h_{target}$), up ($\theta_o > \pi/2$). All three are reachable in the geometry layer since Phase 1 and published as `los_direction`, and since Phase 2 all three **dispatch** in the atmosphere (`atmosphere/topology.py`): `down` takes the backend's own `evaluate` unchanged, `up`/`level` take segment composition. The refusal that remains is a per-backend **capability** one (§7), not a pending-implementation one.
...
| **Sensor below the target** — column `h_sensor → h_tgt`, zenith at `h_sensor` | **A1** | **Supported (Phase 2, 2026-07-26)** on `atmosphere.model = "simple"` — `atmosphere/observer_leg.py` + `segment_simple`; other backends raise a capability error naming what is supported |
| **Equal / near-equal altitudes** — constant-altitude arm | **A5** | **Supported (Phase 2, 2026-07-26)** on `atmosphere.model = "simple"` — `atmosphere/level_arm.py`, judged by the tangent-topology horizon guard; other backends raise the same capability error |
...
**Since Phase 2 (2026-07-26)**: locked decision 21 is implemented. `geometry.solar_zenith_rad` and `AtmosphericGeometry.solar_zenith_rad` span the closed $[0, \pi]$, and whether a point is lit is a **per-altitude** question — `atmosphere/solar_shadow.py`: sunlit iff $(R_E + h)\sin\theta_s \ge R_E$, i.e. $h \ge R_E(\sec\delta - 1)$ for solar depression $\delta$. Sunlit-target-over-dark-ground is therefore expressible (a 60 km booster is lit at 5° depression; the ground below it, shadow height ≈ 24 km, is not). A sunlit target below the terminator gets a two-arm tangent transit for $\tau_{sun}$ (`atmosphere/solar_transit.py`, **provisional** — no MODTRAN twilight deck in batch 1); a shadowed one gets $\tau_{sun} \equiv 0$. Down-looking scenes with $\theta_s \le \pi/2$ are bit-identical to before.
...
| Space, **through** an atmospheric column (up-looking, near-horizontal) | **B2 — `SkyBackground`** | B2 | **Supported (Phase 2, 2026-07-26)**, band-gated (decision 20). Selected by `core/los_termination.py`, which follows the LOS past the target; the down-looking default is untouched |
...
|---|---|---|---|---|---|---|---|---|
| E1 | ground → ground | Two 30 m towers 8 km apart, level LOS (θ_o ≈ 90.04°; both endpoints inside each other's geometric horizon) | LWIR / point_source | Thermal graybody (**T1**) | Horizontal constant-altitude arm (**A5**) | Terrain beyond the target (**B3**, explicit ε_g, T_g) | None (**I0**) | **Supported (Phase 2, 2026-07-26)** on `atmosphere.model = "simple"`. Equal altitudes resolve to a real horizontal triangle — the degenerate collocated carve-out is retired (guardrail G4, GF-1) — and the tangent-topology guard passes this arm clean (Δh ≈ 1.3 m), which is how open question 10 was resolved. Note this row's B3 terrain background is a **user choice**, not the Rule-B default: a level LOS's continuation exits the atmosphere, so the default is B2 and B3 must be supplied explicitly (and, on a level path, is currently refused at `AtmosphereStage` — see §7) |
| E2 | ground → air | Ground site tracking an aircraft at 10 km altitude, ~30 km slant range (θ_o ≈ 110°) | MWIR / point_source | **T3 mandatory** (T1 permitted if ρ ≈ 0, e.g. plume-dominated) | Up-path only, 0 → 10 km (**A1**) | Sky along the LOS (**B2**, MWIR — first-delivery band) | Solar direct + thermal downwelling (**I2**) | **Supported (Phase 2, 2026-07-26)** on `atmosphere.model = "simple"`, end to end from parameters to SNR — pinned by `tests/integration/test_direction_aware_atmosphere.py`. Remaining caveats: metric conditioning is Phase 3 (ground-projection metrics must still be deselected by hand through the Gap 96 toggles), and the other backends raise a capability error. Owner priority 1 |
| E3 | ground → space | Ground SST site observing a satellite above `h_atm_top` | VIS / point_source | Lambertian reflective (**T2**) | Up-path full column, 0 → `h_atm_top` (**A1**); vacuum above | Sky along the LOS (**B2**, VIS — provisional, carries a `UserWarning` per decision 20) | Solar direct at TOA, unattenuated (**I1**) | **Supported (Phase 2, 2026-07-26)** on `atmosphere.model = "simple"` (the full column plus the vacuum leg above it). Remaining caveats: the VIS sky background is **provisional** (decision 20 — the run emits the `UserWarning`), credible spatial performance needs the Gap 110 turbulence upgrade (Phase 3), and metric conditioning is Phase 3. Owner priority 4 |
...
| E5 | air → air | Two aircraft at 10 km, 150 km apart, level LOS (θ_o ≈ 90.7°) | MWIR / point_source | **T3** (T1 if ρ ≈ 0) | Horizontal constant-altitude arm (**A5**) | Sky along the LOS (**B2**), inside the near-horizontal shoulder → computes with the refraction-caveat `UserWarning` (decision 18) | Solar direct at altitude (**I1**) | **Supported (Phase 2, 2026-07-26)** on `atmosphere.model = "simple"` — the level arm carries the observer leg and the scene runs to finite metrics, warning about the unmodelled refraction (Δh ≈ 441 m at 150 km, ≈ 784 m at 200 km). Pinned by `tests/integration/test_direction_aware_atmosphere.py`. Remaining caveats: the refraction thresholds are provisional pending the batch-2 on/off deck pair, the A5 MODTRAN library family is batch 2, and metric conditioning is Phase 3. Owner priority 2 |
| E6 | air → space | Sensor at 12 km viewing a boosting target at 120 km | SWIR / point_source | **T2 escalated to T3** (hot-target rule, `T_t` ≳ 700 K) | Up-path partial column, 12 km → `h_atm_top` (**A1**) | Sky along the LOS with a cold-space terminus (**B2**) | Solar direct at TOA, unattenuated (**I1**) | **Supported (Phase 2, 2026-07-26)** on `atmosphere.model = "simple"`: the observer leg is the 12 km → `h_atm_top` partial column and the continuation above it is vacuum, so `L_sky ≡ 0` and the background reduces to the leg's own emission. Remaining caveats: metric conditioning is Phase 3 |
...
Reading the table since Phase 2 (2026-07-26): every row is **supported**, with the backend named. "Supported on `simple`" means the scene runs end to end on `atmosphere.model = "simple"` and raises an actionable capability error on `modtran` / `interpolated` / `tabulated`, which have no up-looking or ITYPE=1 run families wired into the dispatch. Near-horizontal paths are additionally judged by the horizon guard (§7, `RADIANT_Geometry.md` §4.1). No row describes behavior that is silently wrong — every restriction is loud (§7).
...
**Delivered 2026-07-26** (locked decision 20; plan Phase 2): a fifth variant `SkyBackground` (B2) carries the sky radiance along the LOS for up-looking and near-horizontal terminations, band-gated as ratified (MWIR/LWIR supported; VIS/NIR provisional with a `UserWarning`). It carries **no user parameters** — the radiance is derived from the scene by `AtmosphereStage` and handed to assembly — which is what distinguishes it from `UserSpectralBackground` (the "I have a measured spectrum" door).
...
Rules A and I are AtmosphereStage's surface. **Delivered 2026-07-26 (plan Phase 2)**: the path-segment product (a column evaluated between two arbitrary altitudes with the zenith at the lower endpoint) that yields A1 and A5 (`atmosphere/segments.py`, `segment_simple.py`, `level_arm.py`), the sky-radiance-along-LOS product feeding B2 (`atmosphere/sky_radiance.py`), and the per-altitude shadow-height illumination test (`atmosphere/solar_shadow.py`). Guardrail G1 held: `AtmosphericQuantities` still carries exactly eight fields, and the one genuinely new product (the sky continuation radiance) rides alongside the bundle on `TopologyProducts` rather than being bolted onto it.
...
### 6.3 Direction-general forms — delivered (Phase 2, 2026-07-26)

Delivered on the simple backend with the Category-C truth anchors the plan called for: MODTRAN up-looking K-block runs, reciprocity checks, vacuum limits, and horizontal-path analytic values (`tests/integration/test_segment_modtran_anchors.py`, `test_uplooking_horizontal_anchors.py`, `test_direction_aware_atmosphere.py`).

For an **up-looking** observer leg (A1, sensor below target), the target term keeps its form with the segment endpoints swapped — transmittance is reciprocal, so `τ` over `h_sensor → h_tgt` is the same τ read the other way (computed with the zenith at the lower endpoint, `h_sensor`). The path-radiance term is **not** reciprocal and is the *up-path* product (`SegmentQuantities.L_toward_lower`). No new field was added to `AtmosphericQuantities`: the observer-leg segment fills the existing `tau_up` / `L_path_up` slots, and because the LOS terminates on space the same segment fills `tau_full_up` / `L_path_full` — the background source plane *is* the target plane. The background term is the sky radiance along the LOS beyond the target (B2), terminating in cold space.

For a **horizontal** arm (A5) the column is constant-density: `τ = exp(−α(h)·d)` in the simple model at the local absorption coefficient evaluated at the arm's altitude, over the true spherical chord; MODTRAN `ITYPE=1` (Card 3 `RANGE`, `ModtranConfig.hrange_km`). The near-horizontal guard band of decision 18 applies, and the constant-density assumption's validity is bounded by it: the guard caps the tangent-point depression at 2 km (mean sag ≈ 1.33 km), so the arm is evaluated at the endpoint altitude and the sag-induced **under**-estimate of column density is the dominant modelling error in the shoulder band. The size of that error is species-dependent, and it is the *water* scale height (≈ 2 km) that governs an IR arm: ≈ 3 % at the 100 m clean edge, ≈ 1.9× at the 2 km raise threshold — which is why 2 km raises rather than warns. The authoritative table, per band and per guard step, is `RADIANT_Atmosphere.md` §4.2f.
...
| S8/S10/S10b doors' own pairings reaching only `evaluate()` | S8 + (ε, T); S8 + S10; the two point-intensity modes together; S10 or S10b + (ε, T) | Raise `ParameterBoundsError` at **both** the door and `evaluate()` | **Implemented (CU-293, ex-CU-294, 2026-08-01)** — extracted from `source/_inferrer.py` into `target_spec.py::check_user_radiance_conflicts` / `check_point_intensity_conflicts` / `check_user_intensity_conflicts` and registered in `validate_target_spec`. The refusals themselves are unchanged; what changed is that the GUI's clone-validate edit discipline now rejects them at commit time instead of committing a config it already knows is invalid |
| ε(λ) door (`emissivity_path`) paired with a rival surface | `source.target.emissivity_path` set together with scalar ε, ρ-family, S8, S10, S11 or S12 | Raise `ParameterBoundsError` naming the rival | **Implemented (CU-318, 2026-08-02; symmetric since CU-323, 2026-08-02)** — extracted from `source/_inferrer.py::_load_emissivity_on_grid` into `target_spec.py::check_emissivity_path_conflicts`. The pair unique to this door — `emissivity_path` + scalar `source.target.emissivity` — reached only `evaluate()` before CU-318. Its other nine rivals each open a door that dispatches **first**, so `evaluate()` never reached the guard and discarded the ε(λ) surface in silence (Rule 17, same class as the S11/S12 row above) while the seam refused them; CU-323 extended the CU-293 owner ruling and made the guard a **pre-dispatch** check, run first at both entry points. All ten pairs are now refused at seam and evaluate with identical text; no shipped config newly raises (none sets `emissivity_path`) |
...
| ~~**Up-looking or level path whose lower endpoint is inside the modelled column**~~ | Was the Phase-1 pending-capability refusal | — | **LIFTED (plan Phase 2, 2026-07-26).** `atmosphere/_uplooking_guard.py` is deleted; `atmosphere/topology.py` dispatches on direction instead |
| **Up-looking or level path on a backend that cannot serve it** | The direction-aware segment products are built on the CU-161 simple-model species machinery; MODTRAN tape7-import still needs its own up-looking / ITYPE=1 deck geometry (GF-10, CU-224) | Raise at `AtmosphereStage`, before backend dispatch, naming exactly what *is* supported (simple for any endo path; an up-looking interpolated run family for a vertical endo up-look; any backend for a wholly-vacuum path) | **Implemented (Phase 2)** — `atmosphere/uplooking_quantities.py::supports_uplooking`. **Widened 2026-07-30 (CU-226):** an `InterpolatedAtmosphere` built from an up-looking run family and carrying an `uplooking_companion` is now admitted — the family serves the observer leg, the companion the illumination and sky legs, and the split is declared (`UserWarning` + `topology_provenance["backend_split"]`). A **level** path on such a family is still refused: no rung of a column ladder is a constant-altitude arm |
| **`GroundBackground` on an up-looking or level path** | Rule B: the background is what lies behind the *target*; an ascending LOS leaves the atmosphere, so there is no ground column to propagate through | Raise at `AtmosphereStage`, pointing at `SkyBackground` | **Implemented (Phase 2)** — `atmosphere/stage.py` |
| **Limb-crossing LOS *termination*** (the continuation past the target) | No limb radiance model (B4 declined, GF-11) | Raise | **Implemented (Phase 2)** — `core/los_termination.py` classifies the continuation and raises with the tangent altitude, reusing the Phase-1 interior-tangent machinery; enforced in `source/_inferrer.py` (background selection) and `atmosphere/uplooking_quantities.py` |
| ~~`θ_s ≥ π/2` (sun at or below the horizon)~~ | Was rejected outright: `geometry.solar_zenith_rad` was bounded at 1.5707 rad and `AtmosphericGeometry.__post_init__` raised. *(An earlier revision of this row read "Warn (physical, not error)", which never matched the code.)* | — | **LIFTED (plan Phase 2, 2026-07-26)** by locked decision 21. Both bounds now span the closed `[0, π]`; illumination is a **per-altitude** question answered by `atmosphere/solar_shadow.py` (sunlit iff $(R_E+h)\sin θ_s ≥ R_E$), not by a global bound. `geometry.solar_illumination = "night"` (S0) remains the way to declare an unilluminated scene outright. Sub-horizon τ_sun comes from the provisional two-arm transit of `atmosphere/solar_transit.py`; every `θ_s ≤ π/2` scene is bit-identical to before |
...
| Ground-based sensor; up-looking observer legs | A1 | **SHIPPED — Phase 2, 2026-07-26** on `atmosphere.model = "simple"`; other backends raise a capability error |
| Equal-altitude / horizontal paths | A5 | **SHIPPED — Phase 2, 2026-07-26** on `atmosphere.model = "simple"`; MODTRAN `ITYPE=1` deck support landed, its library family is batch 2 |
| Up-looking space-to-space (LEO → GEO) | A0 | **SHIPPED — Phase 1, 2026-07-26** (the gate was geometry only) |
| Sky-path background for upward-looking sensors | B2 (`SkyBackground`) | **SHIPPED — Phase 2, 2026-07-26**, band-gated (decision 20); VIS/NIR provisional |
| Per-altitude (shadow-height) solar illumination | I0/I1/I2 selection | **SHIPPED — Phase 2, 2026-07-26** (decision 21); the twilight $\tau_{sun}$ transit is provisional pending a batch-2 deck |

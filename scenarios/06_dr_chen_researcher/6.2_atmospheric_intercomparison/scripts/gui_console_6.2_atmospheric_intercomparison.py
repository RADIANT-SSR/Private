# GUI scripting-window script — Scenario 6.2: Atmospheric intercomparison — US Standard, SimpleAtmosphere
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/6.2_atmospheric_intercomparison.gui.yaml
...
#
# NOTE: SimpleAtmosphere at us_standard; the MODTRAN tape7 arm needs staged files.
#
...
# (headless smoke test): python scripts/gui_console_6.2_atmospheric_intercomparison.py
...
try:
    ...
    sensor = Sensor.load(
        ...
        / "inputs"
        / "6.2_atmospheric_intercomparison.gui.yaml"
    )
...
print("=== Scenario 6.2: Atmospheric intercomparison — US Standard, SimpleAtmosphere ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 6.2: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

...
def add_vector_with_arrow(
    ...
    if with_break_mark:
        ...
        seg_a = pv.Line(pointa=tuple(start), pointb=tuple(gap_start)).tube(radius=_TUBE_RADIUS_M)
        seg_b = pv.Line(pointa=tuple(gap_end), pointb=tuple(shaft_end)).tube(radius=_TUBE_RADIUS_M)

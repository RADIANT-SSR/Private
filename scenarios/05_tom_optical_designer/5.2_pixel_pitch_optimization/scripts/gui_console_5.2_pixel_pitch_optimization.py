# GUI scripting-window script — Scenario 5.2: Pixel-pitch sweep — mid pixel-pitch baseline
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/5.2_pixel_pitch_optimization.gui.yaml
...
# (headless smoke test): python scripts/gui_console_5.2_pixel_pitch_optimization.py
...
print("=== Scenario 5.2: Pixel-pitch sweep — mid pixel-pitch baseline ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 5.2: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

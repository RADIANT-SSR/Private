...
_CHAR_WIDTH_PX: float = 6.0  # 10-pt proportional-font average advance
...
def _rendered_char_count(text: str) -> int:
    ...
    def _collapse(match: re.Match[str]) -> str:
        ...
        return _LATEX_BRACES.sub("", inner)
...
def add_leader_label(
    ...
    ``chrome`` supplies the theme-bound connector color and pill fill so the
    annotation matches the app theme (ADR-0007 §3); the label *text* keeps its
    per-primitive family color (semantic).
    """
    ...
    # Pill background from the theme's panel surface so the chip reads as a subtle
    # annotation on either the light or dark app theme.
    tp.SetBackgroundColor(*chrome.label_pill_rgb)
    ...
    leader_color = _hex_to_rgb_float(chrome.leader_line)
    leader = vtk.vtkLeaderActor2D()
    leader.GetPositionCoordinate().SetCoordinateSystemToDisplay()
    leader.GetPositionCoordinate().SetValue(float(anchor_screen_xy[0]), float(anchor_screen_xy[1]))

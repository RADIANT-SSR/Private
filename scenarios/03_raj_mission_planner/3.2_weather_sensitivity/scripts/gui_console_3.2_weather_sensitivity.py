# GUI scripting-window script — Scenario 3.2: Weather sensitivity — baseline clear atmosphere
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/3.2_weather_sensitivity.gui.yaml
...
#
# NOTE: Runner's baseline visibility and precipitable-water values. NIIRS-vs-weather is the scenario's headline, so allow_extrapolated=true opts the baseline into the GIQE-5 rating past its SNR-calibration envelope (CU-166/CU-170).
#
...
# (headless smoke test): python scripts/gui_console_3.2_weather_sensitivity.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "3.2_weather_sensitivity.gui.yaml"
    )
...
print("=== Scenario 3.2: Weather sensitivity — baseline clear atmosphere ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 3.2: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

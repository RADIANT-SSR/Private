# GUI scripting-window script — Scenario 10.1: Ground-to-air MWIR detection — up-looking, sky background
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/10.1_ground_to_air_mwir_detection.gui.yaml
...
#
# NOTE: scene_class ground_to_air; los_direction up; SkyBackground behind the target.
#
...
# (headless smoke test): python scripts/gui_console_10.1_ground_to_air_mwir_detection.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "10.1_ground_to_air_mwir_detection.gui.yaml"
    )
...
print("=== Scenario 10.1: Ground-to-air MWIR detection — up-looking, sky background ===")
...
def _sweep_and_plot():
    center = 0.5235987755982988  # current Lower-endpoint zenith ζ_low [deg]
    ...
    ax.plot(axis, snr, "o-")
    ax.set_xlabel("Lower-endpoint zenith ζ_low [deg]")
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 10.1: SNR vs lower-endpoint zenith ζ_low")
    ax.grid(alpha=0.3)
    return fig
...
def _mutation_demo():
    ...
    print(f"+10% lower-endpoint zenith ζ_low: SNR {before:.2f} -> {after:.2f} [-]")

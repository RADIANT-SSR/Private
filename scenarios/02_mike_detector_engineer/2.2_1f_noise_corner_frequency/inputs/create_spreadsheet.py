...
row = add_param(
    ws1, row, "Quantum efficiency (in-band avg)", 55.0, "%", "Measured at 77 K, 8.0–10.0 µm average"
)
row = add_param(ws1, row, "Dark current", 5.0e5, "e⁻/s", "At 77 K, typical for LWIR HgCdTe")
row = add_param(ws1, row, "Operating temperature", 77.0, "K", "Stirling cooler, ±0.05 K stability")
row = add_param(ws1, row, "Read noise (post-CDS)", 350.0, "e⁻ RMS", "Measured in dark, CDS mode")
row = add_param(
    ws1, row, "Full well capacity", 2.0e7, "e⁻", "90% linearity limit, large-format LWIR ROIC"
)
row = add_param(ws1, row, "ADC resolution", 14, "bits", "14-bit ADC")
row = add_param(ws1, row, "System gain", 1220.0, "e⁻/DN", "Set for 100% FWC at ADC max")
row = add_param(ws1, row, "IPC coupling", 0.8, "%", "Per-neighbor, measured via single-pixel reset")

row += 1
row = add_section(ws1, row, "Timing Parameters")
row = add_param(ws1, row, "Nominal frame rate", 60.0, "Hz", "Baseline staring mode")
row = add_param(
    ws1, row, "Nominal integration time", 0.1, "ms", "100 µs — FWC-limited for 300 K LWIR scene"
)
row = add_param(
    ws1, row, "Dead time per frame", 16.57, "ms", "Readout + reset (16.67 ms frame − 0.1 ms int)"
)
...
row = add_param(
    ws2, row, "Flicker coefficient K", 2.5e4, "e⁻²", "From PSD fit: S(f) = K/f for f < f_corner"
)
row = add_param(ws2, row, "Corner frequency", 200.0, "Hz", "Where 1/f PSD meets white noise floor")
row = add_param(ws2, row, "White noise floor", 350.0, "e⁻ RMS", "Read noise plateau above f_corner")
row = add_param(
    ws2, row, "Measurement method", "PSD fit", "—", "Welch periodogram, 1024-pt FFT, Hann window"
)
row = add_param(ws2, row, "Measurement temperature", 77.0, "K", "Cold shutter closed (dark frames)")

row += 1
row = add_section(ws2, row, "Mike's Notes on 1/f vs. Frame Rate")
ws2.cell(row=row, column=1, value="For a staring array at frame rate f_frame:").border = thin_border
row += 1
ws2.cell(
    row=row,
    column=1,
    value="  f_low = 1/t_total = f_frame (lowest temporal frequency in the frame stream)",
).border = thin_border
row += 1
ws2.cell(
    row=row, column=1, value="  f_high = 1/(2·t_int) (Nyquist frequency within one integration)"
).border = thin_border
row += 1
ws2.cell(row=row, column=1, value="  σ_1f = √(K · ln(f_high / f_low))").border = thin_border
row += 1
ws2.cell(
    row=row, column=1, value="  At 30 Hz, more low-frequency noise is included → higher σ_1f"
).border = thin_border
row += 1
ws2.cell(
    row=row, column=1, value="  At 120 Hz, f_low is higher → less 1/f content → lower σ_1f"
).border = thin_border

row += 2
row = add_section(ws2, row, "Test Frame Rates")
headers = ["Frame Rate [Hz]", "t_frame [ms]", "t_int [ms]", "f_low [Hz]", "f_high [Hz]", "Notes"]
...
row = add_param(ws3, row, "Aperture diameter", 15.0, "cm", "Germanium objective lens")
...
row = add_param(ws3, row, "Optical transmission", 85.0, "%", "Ge lens + ZnSe window, AR-coated")
row = add_param(ws3, row, "Optics temperature", 20.0, "°C", "Ambient ground-based")
row = add_param(ws3, row, "Number of optical elements", 3, "—", "Objective, field lens, window")
...
row = add_param(ws3, row, "Target temperature", 300.0, "K", "Ambient outdoor scene")
row = add_param(ws3, row, "Target emissivity", 0.95, "—", "Painted metal surface")
row = add_param(
    ws3, row, "Background temperature", 295.0, "K", "Ambient background (sky + terrain)"
)

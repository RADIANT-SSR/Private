# GUI scripting-window script — Scenario 7.2: Radiometric calibration — as-built sensor
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/7.2_radiometric_calibration.gui.yaml
...
#
# NOTE: Runner's module-level as-built sensor; the sweep varies blackbody T.
#
...
# (headless smoke test): python scripts/gui_console_7.2_radiometric_calibration.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "7.2_radiometric_calibration.gui.yaml"
    )
...
print("=== Scenario 7.2: Radiometric calibration — as-built sensor ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 7.2: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

...
- **Encircled energy couples spatial and radiometric.** In point and sub-pixel regimes, the signal depends on how much PSF energy falls inside the pixel footprint (`EE_box`). OpticsStage computes the PSF; PlatformStage degrades it (jitter, smear, turbulence) and publishes `EE_box` from the fully degraded PSF (`stage_outputs["platform"]["EE_box"]`) — evaluated at the pixel sampling phase `detector.pixel_phase_mode` selects (Gap 129; default `average`), with `EE_box_centered` and `straddle_factor` beside it — before SpectralIntegrationStage can compute the photoelectron rate.
...
| **PlatformStage** | `effective_psf`, regime, params (smear velocity, jitter, t_int) | **EE_box** (ensquared energy in pixel footprint, from the fully degraded PSF, at the selected pixel sampling phase — Gap 129), `EE_box_centered`, `straddle_factor` | MTF_smear(f), MTF_jitter(f) |
...
| **Point** | Target angular extent ≪ PSF (point source unresolved) | `S = (I_target / R²) × A_collect × τ_atm × τ_opt × QE × **EE_box** × t_int` where I = L × A_target. **EE_box is a direct multiplicative factor** on the signal: only the fraction of PSF energy inside the pixel footprint contributes. The compact target sits on a **full-pixel background pedestal** (`background_e`, computed over Ω_pixel from the at-aperture background frame — Gap 73): this is *not* part of the target signal (`contrast_e = signal_e`) but is real photo-charge that contributes background shot noise and fills the well. It is the same full-pixel formula as the sub-pixel/extended background reference, so the noise budget is continuous across the sub-pixel→point-source boundary. When no background frame is configured, `background_e = 0` (Decision #13). |
...
These six enum positions are the *query* frames of the conversion machinery, and are **not** identical to the `RadiometricFrame` snapshots actually stored in `state.frames` (the atmosphere registers `at_source_target` / `at_source_target_reflected` / `at_aperture_target` / `at_aperture` / `at_aperture_background` / `at_source_background`, optics registers `post_optics`, spectral integration registers `photoelectrons`; source/detector/readout register none. The `at_source_*` frames (Gap 91) are the pre-atmosphere source emission `L_source` — the emitted+reflected radiance leaving the target/background before the up-leg, satisfying `at_aperture_target ≈ τ_up · at_source_target + L_path_up`; they are additive diagnostics that feed no downstream metric. `at_source_target_reflected` is likewise additive: the ρ-proportional part of `at_source_target` — direct solar plus diffuse sky, without the self-emission — published so the reflective GUI view can pair the reflectance with the radiance it produces). In particular there is **no `at_fpa` frame** — the focal-plane irradiance lives only as the optics stage-outputs `nearfield_irradiance_at_fpa` / `stray_light_irradiance_at_fpa` — and `at_target` is a query position reachable through the `τ_atm` transfer factor, not a stored snapshot. The full mechanism (transfer-factor extraction, backward propagation, the saturated-well `1/gain` fallback) is specified in **RADIANT_Reference_Frames.md**.
...
class ChainResult:
    ...
    # --- Full-well saturation status (CU-101; WellStatus carries the
    #     clip state + supporting well-charge numbers, each with units) ---

...
| **Status** | OPEN — **partially landed 2026-07-16**: (a) per-regime relevance metadata authored as `regime:<scene_type>` ParameterDef tags on the source-stage background / contrast-reference / fill-fraction params; (c-partial) the GUI Source form disables irrelevant rows with an explanatory tooltip when a scene type is declared (selector shipped GS-1; declared-vs-derived warning shipped ADR-0008 T2). Remaining: tag the non-source stages where relevance genuinely varies, and badge/filter the All-Parameters tree. |

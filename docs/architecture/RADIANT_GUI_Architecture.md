...
| **Source** | Radiometric inputs (thermal + reflective + scene declaration) | **[SHIPPED — GUI plan Phase PS-1; expanded by GUI Capability Expansion plan GS-1]** `SourceInputsForm`, grouped: **Thermal** (target T/ε + `is_hot_target`), **Reflective (solar)** (`source.target.reflectance` — pure-ρ T2 pathway, mutually exclusive with ε/T by engine design, the editor reject discipline surfaces the actionable error; `geometry.solar_illumination` day/night + solar zenith/azimuth), **Background & contrast reference** (T/ε pairs + `source.background.material` library name), **Scene type & regime** (`source.scene_type` declared + `source.regime_override` force + `fill_fraction`) — all shared `FieldRow`s, one `sensor.set` per edit; editing re-evaluates and the emission spectra + Outputs readout refresh. The ADR-0008 T2 declared-vs-derived warning surfaces in Messages. The `albedo` aliases stay tree-only (never invite an over-specified pair) |
| **Source** | Size / shape / orientation inputs, shown per scenario type | **[SHIPPED — GUI plan Phase PS-1]** the shared `TargetShapePanel` (shape combo from `source.target.shape` enum + `shape_radius_m`/`shape_length_m`/… + `shape_yaw_rad`/`shape_pitch_rad`/`shape_roll_rad`) — the **same** widget the Geometry Schematic tab mounts, editing the one `source.target.shape*` set; nominal-dim seeding on shape-select applies (CU-125). The **per-scenario-type gating** (which inputs are relevant) stays deferred to **[GAP 85]** (mission-type relevance) — v1 shows all inputs, disabling only those a *declared* `source.scene_type` excludes (`regime:<type>` schema tags): the sub-pixel `fill_fraction` off outside sub-pixel, and (Gap 98 D) a **"Target — point source"** tab with the radiant-intensity inputs (`source.target.point_intensity_temperature_K`/`_area_m2`/`_emissivity`, `_band_W_per_sr`) enabled only for a `point_source` scene, with the surface-radiance (ε, T) rows disabled there (a point source is defined by intensity, not radiance × area) |
...
| **Atmosphere** | **After** atmosphere — radiance at aperture | **[SHIPPED]** `result.plot.spectral_source()` — draws `at_aperture_target` + `at_aperture_background` frames (the at-aperture radiances the owner wants shown here now that atmosphere is applied) |
...
| **Performance** | Metric selection + metrics + plots | **[SHIPPED — GUI plan Phase PS-6]** a **Metric selection** card (`PerformanceMetricsForm`, Gap 96) of five checkboxes bound to the `performance.metrics.*` group flags (Radiometric / Spatial-MTF / Interpretability / Sampling / Saturation) — each toggle is one `sensor.set` + `parameterEdited` (one GUI action ↔ one API call), so the host re-evaluates and the metric surface re-renders with only the selected groups (a deselected group stops its *computation*, not just its display) — above the metric surface (`ChainResult.metric_records()`) as an `OutputsReadout` metric summary — every computed metric (SNR/NEDT/NIIRS/GSD/MTF@Nyquist and the full set) with its registry unit, **sectioned under labeled group headings** (Windows-deployment finding 12: the flat ~30-metric alphabetical list was unusable) in a fixed reading order — *Sampling / geometry*, *Spatial / MTF*, *Radiometric*, *Interpretability*, *Saturation* (+ a defensive *Other* for a key outside the taxonomy) — partitioned by the same Gap-96 `radiant.api.metric_groups` taxonomy the selection card toggles, headings shared verbatim with the card's checkbox labels (`metric_format.METRIC_GROUP_HEADINGS` / `grouped_metric_records`; presentation-only — the computed set and every value/unit are unchanged) — above `result.plot.mtf()` (system MTF) and `result.plot.mtf_budget()`. A **result-typed metric failure** (a non-finite value, Rule 17 carve-out) renders as `n/a (<failure_reason>)` from the metric's result object, never a bare `nan`/blank. The metric-selection card is the only editable control (the physics inputs are consumed upstream — terminal stage). Single flat pane |

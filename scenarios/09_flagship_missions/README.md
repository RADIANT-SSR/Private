...
|---|---|---|---|
| 9.1_sentinel2_msi_snr | Sentinel-2 MSI | Reflective-band SNR @ ESA L_ref | Predictions within 1.1–2.0× of flight-measured; the committed envelopes bracket B4 only (B2 and B11 fall outside, B11 inconclusive) — the verdict rests on the implied-throughput inversion in the walkthrough |
...
| 9.4_landsat_oli2_snr | Landsat 9 OLI-2 | 9-band SNR @ L_typ; per-element synthetic coatings (Mode 5); all nine bands as ONE ADR-0010 study with a configured `band_filter` element row (Gap 103 v1.1) | All bands clear requirement; 0.8–2.0× of L8 flight (recalled anchors — see its gaps.md); study reproduces each per-band standalone to rel < 1e-9 |

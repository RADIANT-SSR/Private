...
| **Status** | PARTIAL — the **convenience intensity inputs** are DONE (commit pending): `source.target.point_intensity_temperature_K/_area_m2/_emissivity` (blackbody `I=ε·A·B(λ,T)`) and `point_intensity_band_W_per_sr` (scalar band flux) route to `T7IntensityAtSource` (`radiant.source.converters.point_intensity`), so a point source no longer needs a hand-authored CSV. **Remaining OPEN** (A, C, D below). |
...
| 98 | Point-source workflow doesn't steer to intensity (blackbody+zero-area trap, range re-spec, no GUI); convenience intensity inputs added | M | Point-source / SDA / star-tracker | PARTIAL 2026-07-18 (inputs done; A/C/D open) |

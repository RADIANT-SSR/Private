...
| `s.validate_geometry_modes()` | Raise `GeometrySpecificationError` if a geometry family (viewing / solar / kinematics / LOS rate) holds explicit inputs in **two of its doors**, or the S3 door carries both `ltan_h` and `local_solar_time_h` (CU-377). A pure provenance read over the ADR-0006 manifest (`radiant.geometry.mode_guard`) — no physics, no resolve, so it runs on an incomplete configuration. Deliberately stricter than `evaluate()`, which tolerates two doors that *agree* (ADR-0006 rule 2): the GUI's mode selector is the switch, and its edit guard applies this seam differentially, so a loaded file with an agreeing pair still evaluates while an edit that *introduces* a second door is refused at the door. The error's `context` keys are the conflicting dot-paths. |
| `s.geometry_door_values()` | Mapping dot-path → the value every geometry door of the manifest would carry under the currently resolved scene, in canonical units (CU-377; `radiant.geometry.door_values`): `path_zenith_rad` = ζ_low (the lower-endpoint zenith: θ_o down-looking or level, π − η up-looking), `sensor_off_boresight_rad` = η (or ζ_low when the sensor is the lower endpoint), `ground_range_m`, `elevation_angle_rad` = π/2 − ζ_low, `target_range_m` = slant range; `solar_zenith_rad` = θ_s, `solar_elevation_rad` = π/2 − θ_s; `ground_speed_m_s` and `circular_orbit` (whether the orbit door resolved it); `los_angular_rate_rad_s`. Doors with no inverse (S3 site-and-time, K2 target velocity) read `None`, as do all doors when the scene has no path (coincident endpoints), the solar doors at night, and everything when the configuration cannot resolve. Never raises; never mutates. The GUI shows these in the inactive doors and seeds a newly selected door from them, so a switch re-expresses the scene. |
...
(the row structure is shared — `move_element` / `remove_element` change it for **every**
configuration at once, never for one), and sweeps of a whole set. Tolerances, the element
document's row structure, and the default `wavelength_points` are shared state on the base.

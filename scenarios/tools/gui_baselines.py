...
REGISTRY: list[GuiScenario] = [
    ...
    GuiScenario(
        ...
        build=runner_factory(
            ...
            # Importing the runner derives the per-material radiance CSVs
            # (outputs/derived/); the bare-vehicle case at nadir over the full
            # 8-12 um LWIR band is the reference the camo options compare against.
            lambda m: m.build_sensor(m.radiance_files["Bare vehicle"], 8.0, 12.0, 0.0),
        ),
        metrics=("snr", "contrast_snr", "nedt_K"),
        notes=(
            "Bare oxidized-steel vehicle vs scrub, LWIR FLIR at nadir. The YAML "
            "references a derived radiance CSV regenerated by importing the runner."
        ),
    ),
    GuiScenario(
        id="4.4",

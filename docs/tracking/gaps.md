...
| **Status** | PARTIAL → handed to GUI Phase II (2026-07-16). Sub-part (c)'s **declared-vs-derived regime cross-check** SHIPPED as ADR-0008 T2 (commit `995782f`): an explicit `source.scene_type` that disagrees with the derived regime now surfaces a `UserWarning` in OpticsStage (and the GUI Messages panel). The declared axis (`source.scene_type`) and the force axis (`source.regime_override`) already existed (ADR-0008 Amendment 1). **Remaining (the load-bearing part):** (a) per-regime parameter-**relevance metadata** on the `_schema.py` ParameterDefs and (b) the API relevance surface and (c) the GUI **scenario-type selector + relevance filtering/badging** — authored as `regime × dispatched phenomenology`. This is subjective per-parameter physics judgment (owner-guided) and is **handed to the GUI Phase II** instrument work per `Target_Extent_Geometry_Plan.md` §7 (the plan's own hand-off provision); it is no longer gated on the ADR-0008 migration. |

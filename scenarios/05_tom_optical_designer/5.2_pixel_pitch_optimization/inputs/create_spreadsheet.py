...
row = add_param(ws1, row, "Back focal distance", 185, "mm", "Pupil to focal plane")
row = add_param(ws1, row, "Field of view (full)", 2.4, "deg", "Cross-track, 640 pixels at 18 µm")
row = add_param(
...
row = add_param(
    ws1, row, "WFE (RMS, on-axis)", 0.05, "waves", "At λ_ref = 4.25 µm; Zemax merit function result"
)
row = add_param(ws1, row, "WFE reference wavelength", 4250, "nm", "Design reference wavelength")
row = add_param(
    ws1, row, "Airy disk diameter", 41.6, "µm", "2.44 × λ × f/# at λ = 4.25 µm; first dark ring"
)
...
row = add_param(ws1, row, "Target temperature", 310, "K", "Ground target, daytime")
row = add_param(ws1, row, "Target emissivity", 0.95, "—", "Vegetation/soil")
row = add_param(ws1, row, "Background temperature", 295, "K", "Surrounding terrain")
...
ws2.cell(row=note_row, column=1, value="Q = λ·f/# / p  (sampling parameter)").font = Font(
    italic=True
)
ws2.cell(
    row=note_row + 1,
...
ws2.cell(
    row=note_row + 2, column=1, value="Airy disk diameter = 2.44 × λ × f/# = 41.6 µm at 4.25 µm"
).font = Font(italic=True)
...
requirements = [
    ("GSD (nadir)", "< 10", "m", "Ground sample distance at nadir, drives pixel pitch"),
    ("System MTF at Nyquist", "≥ 0.10", "—", "Minimum acceptable MTF for image interpretability"),
    ("SNR (extended, 310 K target)", "≥ 100", "—", "At nominal integration time"),
    (
        "EE 1×1 (ensquared energy)",
        "≥ 0.30",
        "—",

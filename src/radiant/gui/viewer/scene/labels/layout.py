...
def solve_layout(
    ...
    for cid in range(n_clusters):
        ...
        for k, i in enumerate(members):
            ...
            positions[i] = cluster_anchor_centroid + offset * CENTRAL_CLUSTER_INITIAL_OFFSET_PX

    # Per-label anchor-attraction coefficient: halved for cluster members
    # so the spread state isn't pulled back into the pile.
    attraction_k = np.where(is_clustered, CLUSTERED_ANCHOR_ATTRACTION_K, ANCHOR_ATTRACTION_K)
    ...
    for _step in range(NUM_ITERATIONS):
        ...
        min_dist = (
            np.maximum(
                half_sizes[:, None, 0] + half_sizes[None, :, 0],
                half_sizes[:, None, 1] + half_sizes[None, :, 1],
            )
            * 0.5
        )
        ...
        if np.any(has_mesh_bbox):
            for i in range(n):
                ...
                if gap_x < 0.0 and gap_y < 0.0:
                    ...
                    # Co-located → push along +x as a tiebreaker; else along the offset.
                    unit = np.array([1.0, 0.0]) if dnorm < 1e-6 else delta / dnorm
        ...
        forces[:, 0] += np.where(
            left_dist < 0, -EDGE_REPULSION_K * left_dist / (abs(left_dist) + 1.0) ** 2, 0.0
        )
        forces[:, 0] -= np.where(
            right_dist < 0, -EDGE_REPULSION_K * right_dist / (abs(right_dist) + 1.0) ** 2, 0.0
        )
        forces[:, 1] += np.where(
            bot_dist < 0, -EDGE_REPULSION_K * bot_dist / (abs(bot_dist) + 1.0) ** 2, 0.0
        )
        forces[:, 1] -= np.where(
            top_dist < 0, -EDGE_REPULSION_K * top_dist / (abs(top_dist) + 1.0) ** 2, 0.0
        )
        ...
        positions[:, 0] = np.clip(
            positions[:, 0],
            half_sizes[:, 0] + EDGE_PADDING_PX,
            width - half_sizes[:, 0] - EDGE_PADDING_PX,
        )
        positions[:, 1] = np.clip(
            positions[:, 1],
            half_sizes[:, 1] + EDGE_PADDING_PX,

...
row = add_param(
    ws1, row, "Quantum efficiency (in-band avg)", 72.0, "%", "Measured at 77 K, 3.5–5.0 µm average"
)
row = add_param(ws1, row, "Dark current", 100.0, "e⁻/s", "At 77 K operating temperature")
row = add_param(ws1, row, "Operating temperature", 77.0, "K", "Stirling cooler, ±0.1 K stability")
row = add_param(ws1, row, "Read noise (post-CDS)", 20.0, "e⁻ RMS", "CDS mode, 200-frame ensemble")
row = add_param(
    ws1, row, "Full well capacity", 2.0e6, "e⁻", "Large-well ROIC variant, 90% linearity"
)
row = add_param(ws1, row, "ADC resolution", 14, "bits", "14-bit ADC")
row = add_param(ws1, row, "System gain", 130.0, "e⁻/DN", "Set for 107% FWC at ADC max")
row = add_param(ws1, row, "IPC coupling", 1.0, "%", "Per-neighbor, measured via single-pixel reset")
...
row = add_param(ws2, row, "Aperture diameter", 20.0, "cm", "Cassegrain telescope")
...
row = add_param(ws2, row, "Optical transmission", 80.0, "%", "All elements combined")
row = add_param(ws2, row, "Optics temperature", 20.0, "°C", "Ambient ground-based")
row = add_param(ws2, row, "Number of optical elements", 4, "—", "Primary, secondary, fold, window")
...
row = add_param(
    ws2, row, "Cold target temperature", 200.0, "K", "Cold sky background at high elevation angle"
)
row = add_param(ws2, row, "Hot target temperature", 1500.0, "K", "Jet exhaust plume")
row = add_param(ws2, row, "Target emissivity", 1.0, "—", "Assume blackbody for both")
row = add_param(
    ws2, row, "Background temperature", 280.0, "K", "Average scene background (sky + terrain)"
)
row = add_param(ws2, row, "Background emissivity", 1.0, "—", "Assume blackbody background")
...
row = add_param(
    ws3, row, "Minimum SNR (cold target)", 10.0, "—", "Detection threshold for 200 K target"
)
row = add_param(
    ws3, row, "Maximum well fill (hot target)", 70.0, "%", "Stay in linear range for hot target"
)
row = add_param(ws3, row, "Saturation limit", 90.0, "%", "Absolute maximum before FWC clipping")

row += 1
row = add_section(ws3, row, "Sweep Definition")
row = add_param(ws3, row, "Integration time min", 0.001, "ms", "1 µs — shortest practical")
row = add_param(
    ws3, row, "Integration time max", 50.0, "ms", "50 ms — longest before smear concerns"
)
row = add_param(ws3, row, "Number of sweep points", 50, "—", "Log-spaced sweep")

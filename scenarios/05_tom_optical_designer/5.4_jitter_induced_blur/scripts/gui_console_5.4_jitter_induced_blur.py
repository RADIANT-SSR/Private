# GUI scripting-window script — Scenario 5.4: Jitter tolerance — nominal base config
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/5.4_jitter_induced_blur.gui.yaml
...
#
# NOTE: Runner's module-level base_config; the sweep varies jitter RMS.
#
...
# (headless smoke test): python scripts/gui_console_5.4_jitter_induced_blur.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "5.4_jitter_induced_blur.gui.yaml"
    )
...
print("=== Scenario 5.4: Jitter tolerance — nominal base config ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 5.4: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

# GUI scripting-window script — Scenario 7.3: MTF measurement vs prediction — nominal config
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/7.3_mtf_measurement_vs_prediction.gui.yaml
...
#
# NOTE: MTF bench (VNIR, thermal source) — SNR is ~0 by design; MTF is the metric.
#
...
# (headless smoke test): python scripts/gui_console_7.3_mtf_measurement_vs_prediction.py
...
print("=== Scenario 7.3: MTF measurement vs prediction — nominal config ===")
print(f"Radiometric regime : {regime}")
print(f"mtf at nyquist     : {result.metrics.get('mtf_at_nyquist'):.4g} [-]")
print(f"rer                : {result.metrics.get('rer'):.4g} [-]")
print(f"strehl             : {result.metrics.get('strehl'):.4g} [-]")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 7.3: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

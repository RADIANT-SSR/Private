# GUI scripting-window script — Scenario 1.3: Dual-band MWIR/LWIR — MWIR option at nominal hotspot T
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/1.3_dual_band_mwir_lwir.gui.yaml
...
#
# NOTE: MWIR band at the nominal hotspot temperature from the runner's spec sheet.
#
...
# (headless smoke test): python scripts/gui_console_1.3_dual_band_mwir_lwir.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "1.3_dual_band_mwir_lwir.gui.yaml"
    )
...
print("=== Scenario 1.3: Dual-band MWIR/LWIR — MWIR option at nominal hotspot T ===")
...
def _sweep_and_plot():
    center = 0.025  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 1.3: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

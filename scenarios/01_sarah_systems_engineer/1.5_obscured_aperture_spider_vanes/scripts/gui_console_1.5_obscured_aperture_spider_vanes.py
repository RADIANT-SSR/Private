# GUI scripting-window script — Scenario 1.5: Obscured aperture — 30% obscuration, 4 spiders, 1 cm vanes
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/1.5_obscured_aperture_spider_vanes.gui.yaml
...
#
# NOTE: Baseline central obscuration with a 4-vane spider (1 cm width).
#
...
# (headless smoke test): python scripts/gui_console_1.5_obscured_aperture_spider_vanes.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "1.5_obscured_aperture_spider_vanes.gui.yaml"
    )
...
print("=== Scenario 1.5: Obscured aperture — 30% obscuration, 4 spiders, 1 cm vanes ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 1.5: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

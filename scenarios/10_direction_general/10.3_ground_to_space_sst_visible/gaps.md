...
| **Description** | `topology.evaluate_path_topology` returns `TopologyProducts(quantities=_vacuum_quantities(...))` with `sky_source_radiance = None` when both endpoints are at or above `h_atm_top`. But `source/_inferrer.py::_select_los_termination_background` still selects `SkyBackground()` for the space termination, and `assembly._sky_background_source_emission` refuses to default the missing radiance to zero (correctly, under Rule 17). The result is `ParameterBoundsError: assembly: SkyBackground requires the LOS-continuation sky radiance, but sky_source_radiance is None`. Reproduced by raising `geometry.sensor_altitude_m` to 100 000 m in the nominal config. |

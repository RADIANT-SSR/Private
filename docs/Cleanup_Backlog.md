...
**Symptom** (post-slice-2, 2026-04-25): `pytest -m level0` collects 652/2798; `pytest -m level1` collects 378/2798; `pytest -m level2` collects 11/2798; `pytest -m golden` collects 10/2798. The remaining ~1747 tests run under no level marker (slices 3–5 territory) and so cannot be gated by CI per §3.

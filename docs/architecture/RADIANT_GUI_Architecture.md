...
| **Optics** | Element-train editor (per-element R/T/temperature/geometry, ε derived) | **[SHIPPED — GUI Capability Expansion plan GS-4]** `OpticalElementEditor` in the Optics **Elements** tab — the ADR-0009 D2 declarative-document editor: table rows ⇌ the `optical_elements:` entry dicts (R/T cells take a scalar or a spectral-CSV path); *Apply* = one `Sensor.set_optical_elements` call (io-parser validation, Kirchhoff checks; a rejection shows the actionable dialog and never touches the live sensor); the ε column is **derived read-only** (Rule 5 — 1−R mirrors, cavity/zero refractive, from `preview_optical_elements`); the attached train persists through `Sensor.save`/`load` (ADR-0009 D4) and the optics stage runs full-prescription on the next evaluation. **Coating detail (Gap 116):** selecting a table row draws that element's R/T/ε below the table via one `radiant.api.plot_coating_detail` call — native source grid (full stored extent, not the run band), one autoscaled panel per quantity, evaluation band shaded; the table's unapplied entries pass through `entries=` so a draft row previews before Apply, and a row the io parser rejects shows its actionable message in place of the figure. The pane follows the app's dark/light plot theme (`set_dark`, CU-139 seam). **Per-configuration elements (Gap 103 v1.1, owner-ratified 2026-09-02):** in a **multi-member study** the tab renders the active configuration's **effective train** (`ConfigurationSet.effective_optical_elements(active)` — the shared document with that configuration's replace-by-name overrides swapped in, shared order), rows an override swapped in carry an `overridden — <configuration>` badge in the configured-badge family (the same `configuredBadge` QSS rule as the red "C"), and the coating-detail pane therefore inspects the *effective* element. A **scope control** (`Edit scope:` — *Shared document* / *This configuration*, defaulting to shared) chooses what Apply writes; a single-configuration session shows no control and is byte-for-byte the behaviour above. *Shared document* → one `set_optical_elements` on the set's **base**: overridden rows are shown but **locked**, and their shared entry is written back in their place, so a shared edit neither absorbs an override's values nor disturbs the override. *This configuration* → the table is **diffed** against the shared document (entries normalized through the same `normalize_element_document` the API stores, compared key-order- and int/float-independently, exactly — no tolerance) and exactly the changed entries go to one `set_element_override(active, changed)` call; an entry edited back to shared equality drops out, and an empty diff calls `clear_element_override(active)` so the configuration inherits again. A table the io parser rejects has no meaningful diff and goes to the API whole, which refuses it with the parser's message and the **configuration named**, storing nothing. Add / remove / reorder and the Name cells are disabled in this scope (an override replaces a shared element by name — it never adds or removes one), and Remove is refused on an overridden row in shared scope (removing it would orphan the override). The tab takes the session's `ConfigurationScope` through the one `bind_configuration_scope` fan-out, re-reads on a document change and on every selector switch (`bind_sensor`) but never on a configured-value change (an in-progress table edit is not clobbered), and the window re-materializes the displayed sensor after an element commit so a study's displayed train is never the pre-Apply one |

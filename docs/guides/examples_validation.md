...
(from a run of `scripts/run_external_validation.py`):
...
with $\varepsilon = 1 - R$ derived per Kirchhoff's law, an anti-reflection-coated
...
The re-run reproduces the study-versus-standalone verdict exactly — all
...
The atmosphere backends are measured against a MODTRAN 6 run matrix rather than against
field radiometry. The full record — every table, every test that pins it,

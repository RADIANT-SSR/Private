...
- **2026-04-23**: **Gap G closed — shared CSV loader delivered.** Two-column loader [`load_two_column_csv`](../src/radiant/source/converters/_csv.py) wired into three inferrer call sites: `source.target.reflectance_path` / `albedo_path` (S5/S6 spectral) and `source.target.brightness_temperature_path` (S11 spectral). The loader parses `λ [µm], value` rows, validates monotonic λ, and hands a `SpectralData` on the caller's grid to the existing converters, which keep ownership of per-quantity bounds (0 ≤ ρ ≤ 1 for reflectance/albedo; 0 < T_B ≤ 10000 K for brightness temperature). The 36-cell matrix harness [`test_spec_form_matrix.py`](../tests/integration/test_spec_form_matrix.py) now exercises `albedo_path` at S6-extended and `brightness_temperature_path` at S11-extended; [`_use_case_coverage.json`](../tests/integration/_use_case_coverage.json) `spec_forms` block shows S5/S6/S11 all three scenes `pass`. All 12 spec forms × 3 scene types are fully supported. Only P3 Gap H (automatic `ScalarLambertianReflectance` wrap at assembly) remains open and has no user-facing impact.
- **2026-04-24**: **Gap H closed — `T2Reflective.rho` narrowed to `ReflectanceDescriptor | None`.** The Phase 6 / Q4 framing is fully realized: scalar and CSV ρ inputs wrap into [`ScalarLambertianReflectance`](../src/radiant/core/reflectance.py) at the source-stage boundary ([`reflectance_to_descriptor`](../src/radiant/source/converters/reflectance.py)), and [`AtmosphereStage`](../src/radiant/atmosphere/assembly.py) consumes ρ via the protocol method `reflectance_at(λ, view_dir, illumination_dir)` with unit vectors derived from the `LineOfSightGeometry`.  The MWIR §3.2 warning fires at the adapter wrap site.  Lambertian ignores the vectors so assembly output is bit-identical to the pre-Gap-H path (verified by a spy-descriptor regression test); anisotropic BRDFs landing on the protocol in future work now receive the geometry they need.  Parametrised invariant [`test_gap_h_invariant.py`](../src/radiant/source/tests/test_gap_h_invariant.py) pins the `ReflectanceDescriptor` invariant across all 4 user surfaces (S4 / S6 scalar + S5 / S6 CSV).  All audit gaps (A–H) now closed.

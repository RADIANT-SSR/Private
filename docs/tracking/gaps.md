...
| **Status** | DEFERRED to v1.1 (owner-ratified 2026-07-25, ADR-0010 D-7). **Gating condition**: a workflow that needs two configurations to differ by more than scalar as-built knobs — e.g. a swapped filter/window per band, or two element trains with different coatings. **Re-audit date**: at the next multi-configuration task, or when a scenario asks for a per-band element train. **Re-audit 2026-08-29 (scenario 9.4, Landsat 9 OLI-2)**: the gating condition fired in earnest — nine bands each need their own filter element. Deferral refreshed, not closed: the scenario works around it with one shared *composite butcher-block* filter element (spectrally faithful because the eight 30 m bands don't overlap and per-configuration band edges select the strip — exactly the physical filter assembly), but the pan band overlaps green/red and cannot join the study file; it ships as a standalone config. A per-configuration element document (the suggested fix) would let all nine bands share one study. Next re-audit: next multi-configuration task or v1.1 scoping. **PLANNED 2026-09-01**: `docs/plans/Configuration_Set_Expansion_Plan.md` (owner-triggered — OLI as the acceptance case; cap 8→12 ratified in the same plan; override mechanism and GUI scope pending owner ratification there). |

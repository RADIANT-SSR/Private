# GUI scripting-window script — Scenario 3.5: Nighttime MWIR feasibility — MWIR thermal-emission pixel
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/3.5_nighttime_mwir_feasibility.gui.yaml
...
# (headless smoke test): python scripts/gui_console_3.5_nighttime_mwir_feasibility.py
...
try:
    ...
    sensor = Sensor.load(
        ...
        / "inputs"
        / "3.5_nighttime_mwir_feasibility.gui.yaml"
    )
...
print("=== Scenario 3.5: Nighttime MWIR feasibility — MWIR thermal-emission pixel ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 3.5: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

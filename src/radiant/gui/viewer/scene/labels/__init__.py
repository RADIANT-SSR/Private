...
def _projected_mesh_bbox(
    ...
    if actor is None:
        return None
    from radiant.gui.viewer.scene.labels.layout import (
        MESH_EXCLUSION_PADDING_PX as PAD,
    )
    from radiant.gui.viewer.scene.labels.leader_label import (
...
def remove_from_plotter(plotter: pv.Plotter) -> None:
    ...
    """
    from radiant.gui.viewer.scene.labels._anchors import collect_anchors
    ...
    for anchor in collect_anchors(state_for_names):
        for suffix in ("_text", "_leader", "_dot"):
            ...
            # The actor may not exist (first call, or partial build); remove_actor is
            # idempotent in spirit but raises on a miss in some PyVista versions. The
            # broad suppress is a lift-over from the prototype (CU filed to narrow it when
            # Part B wires the camera-rebuild path that calls this helper).
...
def add_to_plotter(plotter: pv.Plotter, state: SceneState, chrome: SceneChrome) -> None:
    ...
    labels = [
        LeaderLabel(name=a.name, anchor_world=a.anchor_world, text=a.text, color=a.color)
        for a in anchors
    ]
    ...
    for anchor, label in zip(anchors, labels, strict=False):
        ...
        mesh_bbox_px = _projected_mesh_bbox(plotter, _ANCHOR_MESH_ACTORS.get(anchor.name))
        inputs.append(
            LabelLayoutInput(
    ...
    for label, result, anchor_screen in zip(labels, results, anchor_screens, strict=False):
        add_leader_label(
            plotter,
            label,

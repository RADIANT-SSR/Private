...
Every exception the spectral_integration package raises on purpose derives from
...
__all__ = ["SpectralIntegrationValidationError", "SpectralIntegrationStateError"]


class SpectralIntegrationValidationError(RadiantError, ValueError):
    """A ``radiant.spectral_integration`` computation rejected an input value or argument."""


class SpectralIntegrationStateError(RadiantError, RuntimeError):
    """A ``radiant.spectral_integration`` stage was invoked in an invalid chain state.

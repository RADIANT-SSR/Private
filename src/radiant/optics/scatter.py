...
def scatter_mtf_1d(
    ...
    if not 0.0 <= tis < 1.0:
        raise OpticsValidationError(f"scatter_mtf_1d: tis must be in [0, 1), got {tis}.")
    if sigma_halo_m <= 0.0:
        raise OpticsValidationError(
            f"scatter_mtf_1d: sigma_halo_m must be positive, got {sigma_halo_m}."
        )
...
def scatter_kernel_2d(
    ...
    if npix < 1 or npix % 2 == 0:
        raise OpticsValidationError(f"npix must be a positive odd integer, got {npix}")
    if sample_spacing_m <= 0.0:
        raise OpticsValidationError(f"sample_spacing_m must be positive, got {sample_spacing_m}")
    if not 0.0 <= tis < 1.0:
        raise OpticsValidationError(f"scatter_kernel_2d: tis must be in [0, 1), got {tis}.")
    if sigma_halo_m <= 0.0:
        raise OpticsValidationError(
            f"scatter_kernel_2d: sigma_halo_m must be positive, got {sigma_halo_m}."
        )

...
| **Still deferred** | The **geometry-aware** detection-range solve — α varying along a spherical-Earth slant path, τ_atm(R) recomputed per range. Substantially narrowed since: GF-15 (2026-07-27) gave `up`/`level` a piecewise τ(R) that knows where the ray leaves the modelled column, and CU-263 (2026-08-01) routed `down` through the same solver, so every topology whose receding leg is vacuum is now **exact** rather than first-order. What remains is the one un-modellable piece — a continuation still *inside* the column, i.e. an airborne sensor either way up — which is refused with a named reason (Rule 17) and needs the atmosphere stage to publish a segment-resolved extinction profile along the LOS. Effort M; category C. |
...
| **Workaround** | For long-slant-path detection range, the script-side `detection_generic` callback solver still applies (its contract changed in CU-263: it takes a signal-vs-range callable plus the target-free noise floor, not an SNR-vs-range callable). |
...
| **Status** | CLOSED 2026-09-03 (Configuration_Set_Expansion_Plan Phase 2, branch `cfgset/phase2-elements`, owner-approved after two days of live review; delivered by `a306c848` (io/api configured-row model) + `1c5ee72c` / `9ccd7995` (GUI configured rows, commit-on-edit, CU-344 entry faithfulness)). **Delivered as configured element rows** (plan §3a-bis, superseding both the D-7 whole-document idea and a same-day replace-by-name override design): a row of the shared `optical_elements` document configures like a parameter — dense one-complete-entry-per-member (name included; positional row identity), YAML in-place `- configured: {member: entry, …}` form, scripting API `configure_element` / `set_element_for` / `element_for` / `unconfigure_element(keep=)`, GUI red-C rows with commit-on-edit (D-8 per-member editing). ADR-0010 D-7 carries the dated supersession note. Prior deferral history: DEFERRED 2026-07-25 (D-7); re-audit 2026-08-29 fired the gating condition (scenario 9.4's nine OLI-2 bands, composite butcher-block workaround, pan band excluded); PLANNED 2026-09-01. The 9.4 all-bands showcase is plan Phase 3. |
...
| **Status** | **DELIVERED (Geometry-Flexibility Phases 0–5, 2026-07-26 → 2026-07-28)** — the down-looking-only restriction is gone end-to-end: θ_o ∈ [0, π], `h_sensor` on `LineOfSightGeometry`, direction-general modes, horizon guard, up-looking/level paths through real atmosphere on `atmosphere.model = "simple"` (matrix classes E2/E3/E5/E6), turbulence/kinematics/scene-class relevance (Phase 3), the direction-aware GUI (Phase 4: compositions, θ_o/ζ_low arcs, Δh pill, scene-class card), and the Phase-5 validation series `scenarios/10_direction_general/` (one scenario per priority cell, MODTRAN-anchored where batch-1 runs exist, Rule-4 silent on every class). **Residual, tracked separately:** MODTRAN / interpolated up-looking + ITYPE=1 library families are owner-run batch 2 (those backends raise an actionable capability error meanwhile; the ground-to-space anchor is deferred on the same batch), and the validation wave's fidelity findings are CU-253…CU-269 / Gaps 113–115. (Plan archived: `docs/archive/Geometry_Flexibility_Plan.md`; ADR-0011.) |
...
| **Status** | DELIVERED 2026-07-27 (Geometry-Flexibility Phase 3). $C_n^2(h)$ profile family — `atmosphere/cn2_profiles.py` (contract + selector), `cn2_hufnagel_valley.py` (HV, defaults HV-5/7), `cn2_tabulated.py` (measured table, Rule-6 CSV loaded by `loaders.build_cn2_profile` and injected at `stage_outputs["atmosphere_config"]["cn2_profile"]`). Path-weighted $r_0$ — `atmosphere/r0_path.py`: $r_0 = [0.423 k^2 \sec\zeta \int C_n^2 W\,dh]^{-3/5}$ with the lower-endpoint zenith from the Phase-2 `observer_leg` machinery, plane/spherical wave weighting (spherical peaks at the aperture), a closed-form level-arm branch, and direction-aware limits clipped into the atmosphere. Resolution policy — `atmosphere/r0_resolution.py`: direct `atmosphere.r0_m` wins when set (bit-identical), profile-driven otherwise, >1 % disagreement raises `TurbulenceSpecificationError` (CU-093 pattern). The space-observer `ScopeError` documented in `RADIANT_Atmosphere.md` is **retired** (ADR-0011 guardrail G4): a vacuum path yields a finite huge $r_0$ and the MTF term is omitted. **Not included:** anisoplanatism, scintillation, short-exposure MTF, von Kármán outer scale (all listed §7.2). |
...
|---|---|
| **Found in** | Geometry-flexibility audit (`docs/reports/geometry_flexibility_2026-07/`), 2026-07-26. |
| **Status** | DELIVERED 2026-07-27 (Geometry-Flexibility Phase 3). Both doors, provenance-resolved (§8.3 answer 4): direct `geometry.los_angular_rate_rad_s`, or target-velocity vector (`geometry.target_speed_m_s` / `target_heading_rad` / `target_climb_rad`) derived through `geometry/los_rate.py` (relative velocity minus sensor track velocity, perpendicular component over slant range); >1 % disagreement between doors raises (V0–V4 agreement pattern); published as `los_angular_rate_rad_s` + `los_rate_mode` with a new manifest family (GUI auto-follows). Consumer: `platform/relative_motion_smear.py` — one relative-motion smear feeding BOTH Rule-4 paths; with kinematics unset every existing smear number is bit-identical (differential-proof test). |
...
| **Status** | DELIVERED 2026-08-29 (`2ba7951d`) — `radiant.api.plot_coating_detail` (native-grid assembly, `api/coating_detail.py`) + `plot_element_coating` renderer (per-quantity autoscaled panels, evaluation band shaded) + Elements-tab selection-driven detail pane with draft preview (`entries=` override) and actionable-message fallback. |
...
| **Status** | DELIVERED 2026-09-05 (`ca6b4f3e`, v1 — plan Phases 0–2); COMPLETE 2026-09-06 with Phases 3 (GUI, live-review approved) and 4 (up/down counting v1.1 per D6/D7, scenario 2.9 — numbered 2.7 until the CU-362 renumber, 2026-09-21). Plan archived: `docs/archive/Digital_Pixel_Readout_Plan.md`. |
...
| **Status** | DELIVERED 2026-09-06 (`50c191b0`, all plan phases; plan archived). 21 presets + 22 hash-manifested reference PDFs + generated configs (`75e5ad7f`, `6e218b5f`); GUI part-library row + picker with FPA/ROIC kinds, remove-to-custom flow, incomplete-config advisories (branch `gap119/phase3-gui`, nine owner live-review rounds, approved 2026-09-06). Follow-on: Gap 121 (ROIC + detector-material composition, design conversation). |
...
| **Status** | DELIVERED 2026-09-07 (final commit `6152be2f`; phases `1547e44c`/`4fb2e5da`/`d36cf237`/`34bb48b8`) — all plan phases landed (plan archived: `docs/archive/Calibration_Model_Plan.md`). Phases 0–2 (2026-09-06): chain skeleton + schema + `BiasTerm` accumulator (P0); anchored physics modules + D2 handoff (P1); live end-to-end dispatch, post-calibration totals preferred by SNR/CSNR/SCNR, radiometric-accuracy metric, both ADR-0012 guarantees contract-tested on full chains (P2). Scenarios (P4): 2.7_calibration_limited_nedt + the 1.4 TDI calibration variant (CU-346 filed). GUI Calibration screen (P3): live-review approved 2026-09-07 after three passes (blank-config edit prompt + pin reset; stage screens editable pre-evaluate). Follow-ons: Gap 122 (v1.1 extensions), CU-346. | |
...
| **Status** | **CLOSED 2026-09-12 (`44eb0bfe`) — all five items delivered.** Item 1 2026-09-07, **item 2 DELIVERED 2026-09-08 scoped to three points** (branch `gap122/item2-three-point`; owner: "beyond that is usually not done") (`calibration.source_uniformity_K` + `cal_source_uniformity` term, `source_uniformity.py`, Level 0 + stage + chain tests, GUI Source-group field; commit on `gap122/item1-source-uniformity`); **item 3 DELIVERED 2026-09-11** (`calibration.band_center_uncertainty_um` + `spectral_cal` BiasTerm, `spectral_cal.py`, Level 0 + stage + GUI tests, GUI Spectral-cal group; branch `gap122/batch-v11`); **item 4 DELIVERED 2026-09-11** (`cal_path`/`shutter_after_element`/`narcissus_fpn_pct`, `internal_cal.py` photon-weighted per-element split over `nearfield_per_element`, `internal_cal_offset` BiasTerm + `narcissus_fpn` spatial NoiseTerm, Level 0 + stage + GUI tests; branch `gap122/batch-v11`); **item 5 DELIVERED 2026-09-12** (`cal_point_mode` flux_fraction + `cal_flux_low/mid/high`, temperature-anchored inputs rejected as over-spec per the owner-adopted recommendation, CU-346 advisory retired in-mode, GUI mode-switched rows; branch `gap122/item5-flux-door`). |
| **Description** | The Gap 120 v1 model omits four real calibration error sources, all absorbable by the existing term-family + bias-accumulator architecture with no new structure: (1) **cal-source spatial non-uniformity** (blackbody ±0.01–0.05 K across the aperture, imprinted into the correction at cal time → residual FPN ≈ ΔT_unif · dS/dT even AT the cal points, where v1 reports exactly zero); (2) **multi-point NUC** (`n_point` scheme extension — piecewise correction shrinks the nonlinearity parabola per segment); (3) **spectral calibration uncertainty** (band-center/edge drift → scene-temperature-dependent radiance bias, a fourth `BiasTerm`); (4) **internal-cal path mismatch** (shutter/flag cal excludes fore-optics emission → offset bias + narcissus-pattern FPN; the ADR-0012 ops-level growth path); (5) **flux-ratio cal-point door** (added 2026-09-07 at CU-346 closure — a flux-declared cal point, e.g. `calibration.cal_flux_fraction` for integrating-sphere flat fields, superseding the Planck temperature mapping on reflective bands; until it lands, the shipped CU-346 guard warns and the mapping is a documented stand-in). Electronics INL is deliberately absorbed into `calibration.nonlinearity_pct` (documented); FPA-ΔT drift stays under the ratified D4 deferral. |
...
| **Suggested fix** | Item 1: ~~one parameter (`calibration.source_uniformity_K`) + one term into the `nuc_residual` family, effort S — schedule next~~ **delivered 2026-09-07**. Item 2: ~~scheme enum + per-segment residual, effort S–M~~ **delivered 2026-09-08 (three-point only)**. Item 3: one BiasTerm + band-derivative, effort S. Item 4: fore-optics-emission split, effort M–L (needs the optics warm-emission budget by element group). Item 5: one parameter + a cal_points branch, effort S–M (owner-ruled door-later at CU-346, 2026-09-07). Category C throughout. Related: Gap 120 (delivered), CU-346 (reflective-scene cal points). |

## Gap 123: No predictive dark-current law — a blank-sheet HgCdTe study cannot derive J_dark from cutoff and temperature (Rule 07 / Rule 22), and three live docs already claim Rule 07 is shipped

| | |
|---|---|
| **Found in** | Owner question, 2026-09-07 — "how do rule07 and or rule22 figure into the radiometric model? If they aren't in there, should they be?" Investigation found no predictive law in code, while `RADIANT_Master_Architecture.md:130`, `RADIANT_Personas.md:44`, and `RADIANT_Testing_Validation.md:585` claim "Rule 07 dark current" as a shipped Detector capability — aspirational drift from the archived Phase 1 plan, recorded here per Rule 21 (owner-gated) and resolved by this gap's fix. |
| **Status** | OPEN — implementation authorized by owner 2026-09-07; single-slice task, no plan doc. |
| **Description** | Dark current is strictly measured-input: `detector.dark_rate_e_per_s` at a reference temperature with optional user-supplied Arrhenius Eₐ scaling (`detector/dark_current.py`), or a measured J(T) CSV curve (`io/dark_current_csv.py`, `DarkCurrentCurve`). There is no J(λc, T) predictive law, so a notional-MCT design study with no datasheet must invent its dark rate. Rule 07 (Tennant et al., JEM 37, 2008; "Revisited" 2010; LW/VLW revisit 2019) and its modern successor ("Rule 22") are closed-form empirical laws J_dark(λe·T) for p-on-n MBE HgCdTe — formulas, not tables, so they belong in the detector stage as a derivation path, not in the Gap 119 preset library (presets are measured per-part values with provenance). |
...
| **Status** | **DELIVERED 2026-09-08** (commit `5401b04b`, branch `gap126/shipped-examples`, merged after owner review; wheel-verified in a scratch venv — 6 templates + 6 examples discovered off-repo, the CSV-referencing OLI-2 study loads and evaluates from site-packages). |
...
| **Status** | CLOSED 2026-09-11 (delivered 2026-09-09, merge `e0cd1d13` of branch `gap127/element-only-emission`; registry closed 2026-09-11 after a closeout audit found W1–W5 all landed) — delivered per `docs/plans/Optics_Emission_Model_Rules.md` (now archived): `optics.scalar_emissivity`, the `make_lumped_element` emissivity argument, the `OpticalElement.declared_emissivity` field and its Rule-5 carve-out deleted (W1); cavity per-surface T + R = 1 enforced to `_CAVITY_KIRCHHOFF_TOL`, factory + YAML reader accept one of R/T per surface and derive the other (W2); SDA and ground-to-air MWIR templates stripped of the inert keys with a Gap-127 header note (W3); `tests/integration/test_element_only_nearfield.py` replaces the scalar-emissivity test and the weak-absorption ε_eff → α·t Level-0 anchor lives in `optics/tests/test_element.py` (W4); RADIANT_Optics.md §5/§6/§7, parameter reference, and CHANGELOG updated (W5). Follow-ons the plan listed as out of scope are themselves closed: well-fill omission → CU-350 (closed 2026-09-09), MTF overlay accessor → CU-352 (closed 2026-09-10); the surviving descendant is CU-351 (down-phase reference charge, owner-gated). `optics.optics_temperature_K` was subsequently removed 2026-09-10 (inert after this gap). |
...
| **Status** | CLOSED 2026-09-09 (commit `a407ec43`) — delivered per `docs/plans/Nearfield_Etendue_Model.md` (now archived): effective pupil (`optics.cold_stop_undersize_frac`, `optics.cold_stop_obscuration_ratio`) in `optics/effective_pupil.py`, cone-only near-field in `optics/etendue_cone.py` + `optics/nearfield_irradiance.py`, `optics.nearfield_fraction` / `optics.cold_stop_efficiency` / `optics.optics_distance_to_fpa_m` and the element-format `diameter_m` / `distance_to_fpa_m` keys deleted with Gap-128-naming errors, scenarios 7.2/7.5/10.1 retuned to realistic coated trains and 7.4 rebuilt as the undersizing sweep. Defaults (u = 0) are bit-identical to the pre-change engine on every non-element result. Owner-ratified model (2026-09-09, two rounds): (1) in-cone emission uses the system étendue cone Ω_cone = 2π(1 − cos θ), θ = arctan(1/(2N_eff)) — the only geometry; per-element `diameter_m` / `distance_to_fpa_m` are deleted from the emission calculation, element schema/table, and YAML format; (2) E_nf(λ) = Ω_cone · Σ ε_i·B(λ,T_i)·τ_down,i — in-cone emission arrives through the imaging path and cannot be blocked; (3) a cold stop that blocks ALL out-of-cone warm structure is always assumed (owner: "we will always have a cold stop") — no out-of-cone/enclosure term, no shield temperature; `optics.nearfield_fraction` (+ deprecated alias `cold_stop_efficiency`) is deleted; (4) the cold stop is modeled as what it physically is — the aperture stop, slightly undersized for tolerancing (owner ruling): a fractional pupil reduction (and optional obscuration increase) defines the EFFECTIVE pupil, which feeds A_collect, f/#, the diffraction PSF/MTF, and Ω_cone consistently — signal and near-field scale together. Known accepted limitations: field-conjugate (direct-view) elements are treated as pupil-filling; uncooled no-cold-stop cameras are out of the near-field model's scope. |
| **Impact** | Results-affecting for every element-mode scenario: removing the η_cold multiplier raises near-field by 1/η (7.2 ×20, 7.5 ×25, 10.1 ×10), and those scenarios' fallacy-era train emissivities (ε = 0.26–0.32) get realistic per-mirror values in the same pass. Scenario 7.4 is redesigned as a cold-stop undersizing sweep (tolerancing-margin vs signal trade). Removes two element-table columns and one public parameter; adds the cold-stop pupil parameters. |
| **Suggested fix** | Plan: `docs/plans/Nearfield_Etendue_Model.md`. Physics core + schema change + scenario retune; GUI Transmission-tab redesign follows separately (owner ordering: physics first). Effort M–L; Category C. |

## Gap 129: Point-source / sub-pixel EE_box has no selectable pixel-sampling phase (straddle factor) — the chain silently applies the uniform-phase average, the docs describe a pixel-centred PSF, and neither worst-case nor specified-offset conventions are expressible

| | |
|---|---|
| **Found in** | Owner design question, 2026-09-11: "for point sources or sub-pixel targets the performance calculations utilize the ensquared energy on the pixel; there should be an additional phasing / straddle factor for the offset location of the PSF relative to the pixel." Investigation showed the pixel-aperture rect is convolved into the `EffectivePSF` (`optics/stage.py` §6 step 1) *before* `PlatformStage` integrates a pitch-wide box over it — rect ⊛ rect is a triangle, so the shipped `EE_box` is exactly the expectation over a source position uniform across one pitch (verified: Q=2 Airy chain value 0.1606 vs explicit 33×33 phase average 0.1604; PSF-centred analytic anchor 0.1773; corner worst case 0.1321). At Q=1 the spread is centred 0.525 / average 0.392 / corner 0.208. |
...
| **Status** | **CLOSED 2026-09-17 — all five phases delivered; the suite ships.** Phase 5 merge `9063a037`. `docs/plans/Support_Documentation_Plan.md` is complete and archived (`docs/archive/`). Phase 0 (`gap131/phase0-build-infra`, merge `738ad8c0`) built the multi-volume `scripts/build_manual.py`, the shared `scripts/manual_assets/` template on TeX-Live-bundled portable fonts, build-time source validation, ruling-Q2 bound-spec header stripping, the offscreen `scripts/gen_gui_screenshots.py`, and the non-blocking CI `--tex` tripwire (ruling Q5); Phase 1 (merge `3f7e3ef0`) the **Theory Manual v1.0** (91 pp.); Phase 2 (merge `ffdb56b8`) the **Technical Reference v1.0** (161 pp., the three architecture specs bound verbatim per ruling Q2 plus the generated parameter reference); Phase 3 (merge `f856228a`) the **User's Guide v1.0** (97 pp., 12 chapters + menu appendix, owner-reviewed per rendered chapter batch); Phase 4 (merge `b617731a`) the **Worked Examples & Validation v1.0** (162 pp., eight full-depth persona case studies, the all-scenario digest compendium per ruling Q3, the flagship-mission validation chapter and the trade-study cookbook). Phase 5 (branch `gap131/phase5-shipping`) added `scripts/build_release.py`: one command builds all four volumes, stages them into `radiant/manuals/` **and** the 710 git-tracked files of the 51-scenario suite into `radiant/scenarios/` (owner direction 2026-09-17), builds sdist + wheel, **verifies the built wheel** (volume count, scenario count against `git ls-files`, no `__pycache__`/bytecode, no `__init__.py` in the scenario data tree), and assembles `build/release/` — four PDFs, `radiant_scenarios_<version>.zip`, wheel, sdist. Ruling Q1 satisfied both ways: in-wheel and standalone, with the repo committing no PDF (Rule 26). |

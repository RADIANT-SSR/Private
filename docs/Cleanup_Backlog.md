...
**Symptom** (post-slice-4, 2026-04-25): `pytest -m level0` collects 1307/2798; `pytest -m level1` collects 900/2798; `pytest -m level2` collects 402/2798; `pytest -m golden` collects 10/2798. The remaining 179 tests run under no level marker (slice 4b territory: `src/radiant/api/tests/`, `src/radiant/data/tests/`, `src/radiant/source/converters/tests/`) and so cannot be gated by CI per §3.

# GUI scripting-window script — Scenario 7.5: Environmental extremes — 77 K nominal detector
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/7.5_environmental_temp_extremes.gui.yaml
...
#
# NOTE: Nominal 77 K detector (dark 5e4 e-/s, QE 0.78).
#
...
# (headless smoke test): python scripts/gui_console_7.5_environmental_temp_extremes.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "7.5_environmental_temp_extremes.gui.yaml"
    )
...
print("=== Scenario 7.5: Environmental extremes — 77 K nominal detector ===")
...
def _sweep_and_plot():
    center = 0.12  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 7.5: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

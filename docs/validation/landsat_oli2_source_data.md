...
| Butcher-block composite (historical — no config file uses it since 2026-09-03) | sum of the eight non-overlapping 30 m-band strips over the blocking floor — one shared spectral element whose strip each configuration's band edges selected. It was the ADR-0010 D-7 workaround for a study needing per-band filters (Gap 103); with configured element rows (Gap 103 v1.1) every band carries its own `filter_b0N.csv` entry, so the composite is retained as a generated artifact only | — | assumption (synthetic) |

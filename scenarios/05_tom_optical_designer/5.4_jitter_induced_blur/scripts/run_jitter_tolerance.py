...
def main() -> None:
    ...
    print("\n=== Jitter Physics ===")
    print("  Jitter MTF: MTF_jitter(f) = exp(-2pi^2 sigma^2 f^2)")
    print("    where sigma = jitter_rms [rad] x focal_length [m] on the focal plane")
    print("    This is a Gaussian blur -- kills high spatial frequencies exponentially.")
    print("")
    print(f"  At f_Nyquist = {f_nyquist:.0f} cy/m:")
    print("    MTF_jitter = exp(-2pi^2 x sigma_fp^2 x f_Nyq^2)")
    print("")
    print("  Jitter affects MTF and RER but NOT SNR.")
    print("  Jitter is random pointing wander -- it blurs the image but doesn't")
    print("  add or remove photons. Signal and noise are unchanged.")
    print("  NIIRS changes only through the RER term: +3.32 x log10(RER).")
    print("")
    print("  IMPORTANT: this analysis assumes jitter is well-sampled within the")
    print("  integration time (many jitter cycles per frame). If jitter is slow")
    print("  (< 1/t_int), it produces frame-to-frame pointing error, not blur.")
    ...
    for i, jitter_urad in enumerate(jitter_sweep_urad):
        ...
        if i % max(1, n_sweep // 10) == 0:
            print(
                f"  [{i + 1:>3d}/{n_sweep}] jitter = {jitter_urad:>5.1f} urad"
    ...
    for i, res in enumerate(results):
        if i % max(1, n_sweep // 20) == 0 or i == n_sweep - 1:
            print(
                f"  {res['jitter_urad']:>13.1f}  {res['sigma_fp_um']:>13.2f}"
    ...
    if snr_spread < 0.1:
        print("  CONFIRMED: SNR is unchanged by jitter (spread < 0.1 [--]).")
        print("  Jitter blurs the image but doesn't affect photon counts or noise.")
    else:
    ...
    for label, target_delta in thresholds:
        ...
        if found_jitter is not None:
            ...
            print(
                f"  {label}: jitter = {found_jitter:.1f} urad"
                f" (sigma_fp = {sigma_fp:.1f} um = {sigma_pix:.2f} pixels)"
            )
        else:
            print(
                f"  {label}: NOT reached in sweep range"
    ...
    print(
        f"  {'Jitter [urad]':>13s}  {'Frac of IFOV':>12s}  {'sigma_fp [pix]':>14s}"
    ...
    for j_urad in context_jitters:
        ...
        res = results[idx]
        print(
            f"  {j_urad:>13.1f}  {frac_ifov:>12.2f}  {sigma_pix:>14.3f}"
    ...
    if mtf_freq_x is not None and mtf_x is not None:
        ...
        print(
            "  Applying jitter MTF analytically for curve plot: MTF_sys x exp(-2pi^2 sigma^2 f^2)"
        )
        ...
        print("  System MTF curve not available; generating analytic approximation.")
    ...
    print("\n=== Summary ===")
    print(
        f"  Sensor: VNIR Pan, {aperture_m * 100:.0f} cm, f/{f_number:.0f},"
    ...
    print("")
    print("  JITTER BUDGET GUIDANCE:")
    print("    For dNIIRS <= 0.5, jitter must be allocated across all sources:")
    print(
        "    sigma_total^2 = sigma_RW^2 + sigma_solar^2 + sigma_cryo^2"
        " + sigma_struct^2 + sigma_ACS^2"
    )
    print("    Total RSS must stay below the threshold above.")
    print("    Typical allocation: each source gets <= 40% of the total budget.")
    ...
    if baseline_niirs:
        ax1.axhline(
            y=baseline_niirs - delta_niirs_1,
            color="orange",
            linestyle="--",
            linewidth=1.5,
            label=f"dNIIRS = -{delta_niirs_1:.1f}",
        )
        ax1.axhline(
            y=baseline_niirs - delta_niirs_2,
            color="r",
            linestyle="--",
            linewidth=1.5,
            label=f"dNIIRS = -{delta_niirs_2:.1f}",
        )
        ax1.axhline(
            y=niirs_floor,
            color="darkred",
    ...
    ax2.plot(jitter_arr, mtf_arr, "b-o", markersize=3, linewidth=2, label="System MTF@Nyquist")
    ax2.plot(jitter_arr, rer_arr, "r-s", markersize=3, linewidth=2, label="RER")
    ...
    print("  Plot saved: outputs/mtf_rer_vs_jitter.png")
    ...
    print("  Plot saved: outputs/mtf_curves_vs_jitter.png")
    ...
    ax4a.axhline(
        y=0.5, color="orange", linestyle="--", alpha=0.7, label="0.5 pixel (rule-of-thumb limit)"
    )
    ax4a.axhline(y=1.0, color="r", linestyle="--", alpha=0.7, label="1.0 pixel (severe)")
    ...
    print("  Plot saved: outputs/jitter_pixels_and_niirs.png")

    print("  All 4 plots saved to outputs/")
    plt.show()

...
- 2026-07-31 `src/radiant/gui/widgets/matplotlib_canvas.py::MatplotlibCanvas._discard_current` — a queued `draw_idle` fires against a destroyed canvas, printing one uncaught shiboken `RuntimeError` to stderr per full GUI run (test passes; shipped sessions can hit the same ordering on pane figure swaps); fix is XS: stop the pending idle draw before `deleteLater()`. (origin: demoted from CU-313)
...
- 2026-07-31 `src/radiant/atmosphere/cn2_hufnagel_valley.py` (docstring) + `docs/architecture/RADIANT_Atmosphere.md` §7.1 — no guidance that HV's ground strength `A = 1.7e-14 m^(-2/3)` is a sea-level daytime value; the pre-CU-262 "absorb elevation into A" workaround is now obsolete and double-counts. Add a "choosing A for your site" note with the Paranal worked example (`A = 2.70e-15` reproduces 0.80 arcsec at 2635 m). (origin: demoted from CU-304)

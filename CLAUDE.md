...
# Lint (CU-089 widened the gate to tests/; CU-270 to scripts/ and dev_tools/, so the
# linter and the formatter now govern the same four trees):
ruff check src/ tests/ scripts/ dev_tools/

# Formatter (must be in the gate battery — this is the check that was missing):
ruff format --check src/ tests/ scripts/ dev_tools/
```

...
- nearfield_shot = 0 is **correct physics** under the scalar-mode lump assumption (ε = 0); the hand calc also predicts 0, so the row passes legitimately. Under Gap 127 (2026-09-09) a scalar lump emits nothing by rule — to model warm optics, declare an `optical_elements:` list whose mirrors carry ε = 1 − R.

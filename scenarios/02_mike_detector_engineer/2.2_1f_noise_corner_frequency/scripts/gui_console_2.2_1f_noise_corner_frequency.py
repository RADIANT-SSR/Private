# GUI scripting-window script — Scenario 2.2: 1/f noise vs frame rate — nominal config
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/2.2_1f_noise_corner_frequency.gui.yaml
...
#
# NOTE: Runner's default make_config (no flicker); the sweep varies frame rate.
#
...
# (headless smoke test): python scripts/gui_console_2.2_1f_noise_corner_frequency.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "2.2_1f_noise_corner_frequency.gui.yaml"
    )
...
print("=== Scenario 2.2: 1/f noise vs frame rate — nominal config ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 2.2: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

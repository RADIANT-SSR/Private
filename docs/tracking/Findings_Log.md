...
- 2026-08-01 `src/radiant/performance/folded_mtf.py:139-143` — `alias_fraction = (folded − optical)/folded` has no absolute floor, so for optics that cut off below Nyquist it divides ~1e-16 by ~1e-16 and reports float noise (the dual-band LWIR configuration prints 0.944314 where the physical answer is 0); the CU-209 fix deliberately anchors the oversampled case on the absolute folded value instead, leaving the floor question open. Results-affecting on one reported metric — promote to a CU if the owner wants the number fixed rather than caveated. (origin: CU-209)

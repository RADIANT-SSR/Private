# GUI scripting-window script — Scenario 7.1: NEDT reconciliation — primary target temperature
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/7.1_nedt_reconciliation.gui.yaml
...
#
# NOTE: Nominal (primary) target temperature 298.15 K (25 C).
#
...
# (headless smoke test): python scripts/gui_console_7.1_nedt_reconciliation.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "7.1_nedt_reconciliation.gui.yaml"
    )
...
print("=== Scenario 7.1: NEDT reconciliation — primary target temperature ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 7.1: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

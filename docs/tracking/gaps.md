...
| **Status** | OPEN — plan ratified by the owner 2026-09-06 (`docs/plans/Calibration_Model_Plan.md`, Active; D1–D7 adopted as recommended). CORE DELIVERED — Phases 0–2 landed 2026-09-06: chain skeleton + schema + `BiasTerm` accumulator (P0); anchored physics modules + D2 handoff (P1); live end-to-end dispatch, post-calibration totals preferred by SNR/CSNR/SCNR, radiometric-accuracy metric, both ADR-0012 guarantees contract-tested on full chains (P2). Scenarios delivered (P4, 2026-09-06): 2.7_calibration_limited_nedt (parabolic floor + drift + accuracy-beside-precision) and the 1.4 TDI calibration variant (SNR plateaus at the correlated floor; optimum changes qualitatively; CU-346 filed on the reflective-scene cal-point anchor). Remaining: GUI screen merge (P3, live-review gated) + closure (P5). |

# GUI scripting-window script — Scenario 5.5: Stray light — clean (no veiling glare) baseline
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/5.5_stray_light_veiling_glare.gui.yaml
...
#
# NOTE: Rooftop target reflectance, zero veiling-glare (clean reference). GUI baseline shortens integration to 1 ms (well ~75% vs 3.75x over-full at the runner's 5 ms) and uses a well-matched gain (8 e-/DN) so neither the 0.3 Me- well nor the 16-bit ADC clips (CU-170). NIIRS stays absent — the extrapolated rating (>11 at SNR ~470) is outside any physical NIIRS range, so it is not opted in (CU-166).
#
...
# (headless smoke test): python scripts/gui_console_5.5_stray_light_veiling_glare.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "5.5_stray_light_veiling_glare.gui.yaml"
    )
...
print("=== Scenario 5.5: Stray light — clean (no veiling glare) baseline ===")
...
def _sweep_and_plot():
    center = 0.15  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 5.5: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

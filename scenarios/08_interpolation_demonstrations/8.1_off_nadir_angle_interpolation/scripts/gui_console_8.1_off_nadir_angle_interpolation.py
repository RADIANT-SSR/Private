# GUI scripting-window script — Scenario 8.1: Off-nadir interpolation — query angle, SimpleAtmosphere
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/8.1_off_nadir_angle_interpolation.gui.yaml
...
#
# NOTE: Query off-nadir angle; SimpleAtmosphere (tape7 family not portable).
#
...
# (headless smoke test): python scripts/gui_console_8.1_off_nadir_angle_interpolation.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "8.1_off_nadir_angle_interpolation.gui.yaml"
    )
...
print("=== Scenario 8.1: Off-nadir interpolation — query angle, SimpleAtmosphere ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 8.1: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

# GUI scripting-window script — Scenario 8.2: Target-altitude interpolation — LWIR, SimpleAtmosphere
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/8.2_target_altitude_interpolation.gui.yaml
...
#
# NOTE: Nominal LWIR chain; SimpleAtmosphere (tape7 family not portable).
#
...
# (headless smoke test): python scripts/gui_console_8.2_target_altitude_interpolation.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "8.2_target_altitude_interpolation.gui.yaml"
    )
...
print("=== Scenario 8.2: Target-altitude interpolation — LWIR, SimpleAtmosphere ===")
...
def _sweep_and_plot():
    center = 0.2  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 8.2: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

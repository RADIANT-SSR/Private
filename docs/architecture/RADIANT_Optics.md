...
**How `scalar_rms` enters the pupil** (CU-355, owner-ratified 2026-09-11): the one-number RMS budget is expanded over the eight image-degrading low-order Noll terms Z4–Z11 (defocus, astigmatism ×2, coma ×2, trefoil ×2, primary spherical) with **equal RMS per term** — `optics/scalar_rms.py::scalar_rms_zernike_coeffs`. The set is a fixed, documented constant (all-positive, equal magnitude; no RNG anywhere), so identical inputs give bit-identical pupils. Noll orthonormality makes the piston-removed pupil RMS exactly the budget on the unobscured disk; on an annular pupil the set is renormalized numerically (fixed internal 512² quadrature grid, independent of the caller's pupil sampling) so the realized RMS over the transmitting annulus still equals the budget. The expansion then flows through the same `make_pupil_phase_zernike` path as explicit-Zernike input, inheriting the λ_ref → λ_op rescale above. Consequence: two sensors with the same budget get the same aberration *mix*, differing only in scale — an analyst wanting a specific mix uses `zernike` mode. (The pre-CU-355 model was a per-pixel white-noise screen: correct Strehl, but all variance at the pupil-grid sample scale — a far-halo scatter with grid-dependent shape — and no wavelength scaling at all.)

...
| **Up-looking or level path on a backend that cannot serve it** | The direction-aware segment products are built on the CU-161 simple-model species machinery; MODTRAN/interpolated need their own up-looking / ITYPE=1 run families (GF-10) | Raise at `AtmosphereStage`, before backend dispatch, naming exactly what *is* supported (simple for any endo path; any backend for a wholly-vacuum path) | **Implemented (Phase 2)** — `atmosphere/uplooking_quantities.py::supports_uplooking` |

...
| CLI | none structural | `radiant run --configuration` and `radiant validate` already route through `ConfigurationSet`. (As built, 2026-09-02: a file-borne bad override fails at **load** with the owning configuration named — earlier than the per-member validate lines this row anticipated; per-member lines carry element errors only for overrides orphaned programmatically after load) |

...
@pytest.mark.parametrize(
    ("band", "expected"),
    [
        ((3.7, 4.8), (0.4494, 0.0944, 0.808)),  # interior of 3.50–5.00 µm
...
def test_edge_value_is_the_mean_of_the_two_regions(edge_um: float) -> None:
    ...
    = 0.0635 per cm^b; floor_od = (0.1375 + 0.0402)/2 = 0.08885 (both
    rows shipped at 0.0000 until the CU-335 re-fit, and at 0.1597 /
    0.0517 until CU-336 corrected the fit's grid convention); b_h2o
    = (0.874 + 0.434)/2 = 0.654.
    """
    lo_region, hi_region = _region_pair(edge_um)

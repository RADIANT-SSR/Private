...
| **Still deferred** | The altitude duplicate (`geometry.sensor_altitude_m` vs `platform.h_sensor`) → **CU-090** (`h_sensor` has stop-gap space-subcase semantics + 20+ call sites; needs an audited collapse or fold-and-delete alongside the SensorDescriptor ADR). `core/repeat_ground_track.py` (revisit/sun-sync) remains unwired — coverage-metric surfacing is a GUI-phase feature. |

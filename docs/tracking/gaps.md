...
| **Status** | CODE FIXED 2026-07-18 (commit `1d212e8`); 29–100 km DATA remainder planned (`MODTRAN_Boost_Ladder_Expansion_Plan.md`) — see **Fix** row |
...
| **Status** | PARTIAL — **inputs done** (`d560507`): `point_intensity_temperature_K/_area_m2/_emissivity` (blackbody) + `point_intensity_band_W_per_sr` (scalar band flux) → `T7IntensityAtSource`. **A + C done** (commit pending): (A) the point_source signal now raises an actionable error that steers to the intensity inputs (not to `projected_area_m2`) when no intensity is present; (C) `source.range_m` falls back to the GeometryStage-derived `slant_range_m` when `geometry.target_range_m` is unset. **Remaining OPEN: D** (no GUI surface for the intensity inputs; Source instrument still shows the surface-radiance params in point_source regime). |
...
| 98 | Point-source workflow doesn't steer to intensity (blackbody+zero-area trap, range re-spec, no GUI); convenience intensity inputs added | M | Point-source / SDA / star-tracker | PARTIAL 2026-07-18 (inputs+A+C done; D/GUI open) |

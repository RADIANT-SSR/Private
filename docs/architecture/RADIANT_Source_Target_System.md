...
**Declared extent is refused at this door (CU-256, owner ruling 2026-07-29).** Any of the three intensity surfaces set together with a declared target extent — `geometry.target.projected_area_m2` or `geometry.target.shape` — raises `ParameterBoundsError` naming which surface to drop. An intensity has the area already integrated out; a declared extent says the target is resolved. They are two mutually exclusive descriptions of one target, and the T7 path can only honour the first: it publishes a fictitious reference area `A_fict` so the ADR-0004 algebra cancels, which silently discarded the declared extent *and* disarmed the matrix §7 point-source validity guard (the published `angular_extent_rad` was the ~1e-11 rad sentinel `√A_fict/d`, not the real extent). `source.target.point_intensity_area_m2` is **not** an extent surface — it belongs to the door. The guard lives in `radiant.source.target_spec.check_intensity_door_extent_conflicts` and runs both at evaluate time (from the T7 door builders, before the sentinels are published) and at resolve time via `Sensor.validate_target_spec()`.

...
def main() -> None:
    ...
    print("\n=== Sampling Analysis ===")
    print("  Q = λ_center × f/# / p    (sampling parameter)")
    print("  GSD = p × h / f           (ground sample distance)")
    print(f"  Airy disk = 2.44 × λ × f/# = {airy_diam_um:.1f} µm")
    print("  Nyquist frequency = 1 / (2p)")
    print("")
    print(
        f"  {'Pitch [µm]':>12s}  {'Q [—]':>8s}  {'GSD [m]':>10s}  {'f_Nyq [cy/mm]':>14s}  {'Airy/pitch':>12s}  {'Sampling'}"
    )
    ...
    for p_um in pitches_um:
        ...
        print(
            f"  {p_um:>12.0f}  {Q:>8.2f}  {gsd_m:>10.1f}  {f_nyq_per_mm:>14.1f}  {airy_ratio:>12.1f}  {regime}"
        )
    ...
    for idx, p_um in enumerate(pitches_um):
        ...
        print(
            f"  {p_um:>5.0f} µm  Q={Q:.2f}  GSD={gsd_m:.1f} m  "
    ...
    print("\n=== Radiometric Regime ===")
    print("  All configurations use EXTENDED regime (310 K ground fills the pixel).")
    print("  Background temperature (295 K) is used only for contrast SNR.")
    print("")
    print("  PIXEL PITCH TRADE-OFF PHYSICS:")
    print("    Pixel pitch affects the system through three competing mechanisms:")
    print("")
    print("    1. SPATIAL RESOLUTION — smaller pixels give finer GSD:")
    ...
    print("")
    print("    2. SAMPLING (Q PARAMETER) — determines aliasing regime:")
    print(f"       Q = λ·f/# / p  at λ = {lambda_center_um:.2f} µm, f/# = {f_number:.0f}")
    print(f"       The Airy disk diameter is {airy_diam_um:.1f} µm.")
    print("       When p < Airy/2 (Q > ~1.2), the PSF is well-sampled.")
    print("       When p > Airy (Q < ~0.6), severe aliasing occurs.")
    print("")
    print("    3. SENSITIVITY (SNR) — larger pixels collect more photons:")
    print("       Signal ∝ Ω_pixel = (p/f)² → signal ∝ p²")
    print("       Signal shot noise ∝ √signal ∝ p")
    print("       SNR ∝ signal/noise. In photon-noise-limited regime, SNR ∝ p.")
    print("       Smaller pixels sacrifice SNR for resolution.")
    ...
    print(
        f"  {'Pitch':>8s}  {'Q':>6s}  {'GSD [m]':>8s}  {'MTF@Nyq':>8s}  {'EE 1×1':>8s}  {'SNR':>8s}  {'GSD<10m':>8s}  {'Verdict'}"
    )
    print(
        f"  {'-' * 8}  {'-' * 6}  {'-' * 8}  {'-' * 8}  {'-' * 8}  {'-' * 8}  {'-' * 8}  {'-' * 10}"
    )
    ...
    for r in results_table:
        ...
        print(
            f"  {r['pitch_um']:>8.0f}  {r['Q']:>6.2f}  {r['gsd_m']:>8.1f}  {r['mtf_nyq']:>8.4f}  "
    ...
    for r in results_table:
        if (
            r["gsd_m"] < 10.0
            and r["mtf_nyq"] >= MTF_REQ
            and r["ee_1x1"] >= EE_1x1_REQ
            and r["snr"] >= SNR_REQ
        ):
            fom = r["snr"] / r["gsd_m"]
    ...
    if compliant:
        print("  Compliant candidates (pass all requirements):")
        print(
            f"  {'Pitch [µm]':>12s}  {'Q':>6s}  {'GSD [m]':>8s}  {'SNR':>8s}  {'NIIRS':>6s}  {'NEDT [K]':>9s}  {'FoM (SNR/GSD)':>14s}"
        )
        ...
        for r, fom in compliant:
            ...
            print(
                f"  {r['pitch_um']:>12.0f}  {r['Q']:>6.2f}  {r['gsd_m']:>8.1f}  {r['snr']:>8.1f}  {niirs_s:>6s}  {nedt_s:>9s}  {fom:>14.1f}"
            )
        ...
        print("  WARNING: No candidate meets all requirements simultaneously!")
        print("  Consider relaxing GSD, MTF, EE, or SNR thresholds.")
    ...
    ax2a.plot(
        pitches, mtfs, "o-", color="#4472C4", linewidth=2, markersize=8, label="MTF at Nyquist"
    )
    ax2a.plot(pitches, ee1s, "s-", color="#70AD47", linewidth=2, markersize=7, label="EE 1×1")
    ax2a.plot(pitches, ee3s, "^-", color="#A5A5A5", linewidth=1.5, markersize=6, label="EE 3×3")
    ...
    ax3a.plot(pitches, snrs, "o-", color="#4472C4", linewidth=2, markersize=8, label="SNR")
    ax3a.axhline(SNR_REQ, color="red", linestyle="--", linewidth=1)
    ax3a.annotate(
        f"SNR req ≥ {SNR_REQ:.0f}",
        (pitches[-1], SNR_REQ),
        textcoords="offset points",
        xytext=(-80, 8),
        fontsize=9,
    ...
    ax4.set_title("Trade Space: SNR vs. GSD (color = Q parameter)", fontsize=13, fontweight="bold")
    ...
    for i, r in enumerate(results_table, 4):
        vals = [
            r["pitch_um"],
            round(r["Q"], 2),
            round(r["signal_shot"], 1),
            round(r["bkg_shot"], 1),
            round(r["nf_shot"], 1),
            round(r["dark_shot"], 1),
            round(r["read_noise"], 1),
            round(r["total_noise"], 1),
            round(r["snr"], 1),
        ]
        for col, v in enumerate(vals, 1):
    ...
    if compliant:
        ...
        summary_items.append(
            ("Optimal pixel pitch", f"{best_r['pitch_um']:.0f} µm (Q={best_r['Q']:.2f})")
        )
        summary_items.append(("Rationale", "Best SNR/GSD figure of merit among compliant options"))

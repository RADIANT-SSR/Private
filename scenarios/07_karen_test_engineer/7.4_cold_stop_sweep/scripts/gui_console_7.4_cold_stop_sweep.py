# GUI scripting-window script — Scenario 7.4: Cold-stop sweep — baseline (unshuttered) config
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/7.4_cold_stop_sweep.gui.yaml
...
#
# NOTE: Runner's module-level baseline config; the sweep varies cold-stop f-number. GUI baseline uses a well-matched gain (512 e-/DN vs the runner's 2.5) so the 14-bit ADC spans the 8.5 Me- well instead of clipping (CU-170).
#
...
# (headless smoke test): python scripts/gui_console_7.4_cold_stop_sweep.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "7.4_cold_stop_sweep.gui.yaml"
    )
...
print("=== Scenario 7.4: Cold-stop sweep — baseline (unshuttered) config ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 7.4: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

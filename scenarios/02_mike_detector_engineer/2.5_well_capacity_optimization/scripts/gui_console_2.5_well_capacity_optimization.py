# GUI scripting-window script — Scenario 2.5: Well-capacity trade — 300 K scene, 1 ms integration
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/2.5_well_capacity_optimization.gui.yaml
...
#
# NOTE: Nominal 300 K scene temperature at the 1 ms comparison integration time.
#
...
# (headless smoke test): python scripts/gui_console_2.5_well_capacity_optimization.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "2.5_well_capacity_optimization.gui.yaml"
    )
...
print("=== Scenario 2.5: Well-capacity trade — 300 K scene, 1 ms integration ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 2.5: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

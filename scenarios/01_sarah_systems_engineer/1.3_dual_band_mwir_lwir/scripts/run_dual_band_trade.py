...
eps_forest = {b: forest.band_averaged_emissivity(*bands[b]) for b in bands}
...
def build_sensor(band: str, hotspot_T: float) -> Sensor:
    ...
    s.set("spectral_integration.integration_time_s", d["Integration time (fire mode)"], unit="ms")
...
def main() -> None:
    ...
    print(
        f"  Range: {forest.wavelength_um[0]:.1f}–{forest.wavelength_um[-1]:.1f} µm, "
    ...
    print(
        f"    MWIR {bands['MWIR'][0]:.1f}–{bands['MWIR'][1]:.1f} µm: "
        f"ε = {eps_forest['MWIR']:.4f} [--]"
    )
    print(
        f"    LWIR {bands['LWIR'][0]:.1f}–{bands['LWIR'][1]:.1f} µm: "
    ...
    contrast_e = {
        b: results[b].stage_outputs["spectral_integration"]["contrast_e"] for b in results
    }
    ...
    print(f"\n{'=' * 95}")
    print(
        f"  SIDE-BY-SIDE AT T_HOTSPOT = {T_nominal:.0f} K "
    ...
    rows = [
        ...
        (
            "Contrast SNR [--]",
            *(
                f"{results[b].metrics.get('contrast_snr', float('nan')):.1f}"
                for b in ("MWIR", "LWIR")
            ),
        ),
        ("SCNR (incl. clutter) [--]", *(f"{scnr[b]:.1f}" for b in ("MWIR", "LWIR"))),
        (
            "NEDT [mK] (Gap 43 approx.)",
            *(
                f"{results[b].metrics.get('nedt_K', float('nan')) * 1e3:.1f}"
                for b in ("MWIR", "LWIR")
            ),
        ),
    ]
    ...
    for b in ("MWIR", "LWIR"):
        ...
        print(
            f"  {b}: band-integrated ΔL(600 K) = {dl_band:8.2f} W/m²/sr over {lo:.1f}–{hi:.1f} µm"
        )
    print(f"  The 600 K Planck peak sits at {2898.0 / 600.0:.1f} µm — inside MWIR.")
    print("  Per-pixel contrast also scales with the photon energy and the")
    print("  detector parameters, which is why the chain comparison above, not")
    print("  ΔL alone, decides the trade.")
    ...
    print("  Detection: P_d from the Gaussian threshold model at P_fa = 1e-6")
    ...
    for T in T_sweep:
        ...
        for band in ("MWIR", "LWIR"):
            ...
            s_val = scnr_of(r)
            saturated = (
                r.stage_outputs["readout"]["signal_e_final"]
        ...
        print(
            f"  {T:>10.0f}  {row[0][0]:>10.1f} {'YES' if row[0][1] else 'no':>5s}  "
    ...
    if sat_note_bands:
        for b in sat_note_bands:
            ...
            print(
                f"  {det[b]['Full well capacity']:,.0f} e⁻ well even in fire mode) — beyond that the"
            )
            print("  radiometry clips and fire temperature cannot be retrieved in-band.")
    ...
    ax1.semilogy(wl_um, np.maximum(dL_600, 1e-4), "k-", linewidth=2, label="ΔL(λ), T_fire = 600 K")
    ...
    ax1.set_title("Fire-to-Forest Spectral Contrast (hand Planck, ASTER ε_bg(λ))", fontsize=13)
    ...
    for b, color in [("MWIR", "tab:orange"), ("LWIR", "tab:red")]:
        ...
        ax2b.plot(T_sweep, [p_detect(v) for v in scnr_arr], "o-", color=color, linewidth=2, label=b)
    ax2a.axhline(
        SCNR_THRESHOLD,
        color="gray",
        linestyle="--",
    ...
    terms_plot = [
        t for t in all_terms if noise["MWIR"].get(t, 0) > 0.01 or noise["LWIR"].get(t, 0) > 0.01
    ]
    x = np.arange(len(terms_plot))
    w = 0.38
    ax3.bar(
        x - w / 2,
        [noise["MWIR"].get(t, 0) for t in terms_plot],
        w,
        color="tab:orange",
        edgecolor="black",
        linewidth=0.5,
    ...
    ax3.bar(
        x + w / 2,
        [noise["LWIR"].get(t, 0) for t in terms_plot],
        w,
        color="tab:red",
        edgecolor="black",
        linewidth=0.5,
    ...
    for i, T in enumerate(T_sweep, 2):
        vals = [
            float(T),
            round(sweep_scnr["MWIR"][i - 2], 2),
            str(sweep_sat["MWIR"][i - 2]),
            round(sweep_scnr["LWIR"][i - 2], 2),
            str(sweep_sat["LWIR"][i - 2]),
    ...
    print(
        f"  Forest ε from ASTER: {eps_forest['MWIR']:.3f} (MWIR) / {eps_forest['LWIR']:.3f} (LWIR)"
    )
    print("\n  At the nominal 600 K fire:")
    print(
        f"    MWIR: SCNR = {scnr['MWIR']:.1f}, well fill {well['MWIR']:.0f}%, "
        f"P_d = {p_detect(scnr['MWIR']):.3f}"
    )
    print(
        f"    LWIR: SCNR = {scnr['LWIR']:.1f}, well fill {well['LWIR']:.0f}%, "
    ...
    print("       and Planck contrast at fixed ΔT grows steeply at short wavelengths")
    print("       (Wien side) — MWIR fire contrast is enormous.")
    print("    2. LWIR sees the 300 K background ~10× brighter (background photon")
    print("       noise + clutter up) while its fire contrast grows only ~8× from")
    print("       300→600 K — LWIR is the mapping band, not the detection band.")
    print("    3. Dynamic range flips the argument at high T: MWIR saturates first")
    print("       (see sweep) — a real fire product pairs MWIR detection with LWIR")
    print("       (or sub-frame integrations) for temperature retrieval.")
    print(
        f"    4. Same canopy, different ε per band (ASTER: "
        f"{eps_forest['MWIR']:.3f} vs {eps_forest['LWIR']:.3f}) — a scalar"
    )
    print("       emissivity shared across bands would bias the background by ~3%.")
    print("\n  RECOMMENDATION: MWIR for detection (higher SCNR at all fire")
    print("  temperatures below saturation); add LWIR or sub-frame MWIR modes if")
    print(
        f"  fire-temperature retrieval above ~{T_sweep[sweep_sat['MWIR'].index(True)] if any(sweep_sat['MWIR']) else 1200:.0f} K matters."
    )

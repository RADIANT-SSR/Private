...
| `TransmissionConfigIncompleteError` | `OpticsValidationError` | `optics.errors` | A transmission input mode selected without its inputs (`key_elements` with no element, `spectral_file` with no curve, …) — a mid-switch state the GUI routes as an advisory. *"resolve_transmission: the key_elements transmission mode needs at least one optical element, and this configuration carries none. Add the elements on Optics ▸ Transmission …"* |
...
| `SpectralIntegrationStateError` | `RadiantError`, `RuntimeError` | `spectral_integration.errors` | The ensquared-energy invariant violated upstream. *"EE_box != 1.0 but regime is 'extended' (EE_box=0.82). In extended-scene mode, EE_box must not be applied. This is a programming error in PlatformStage."* |

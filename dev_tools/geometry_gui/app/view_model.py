...
No plotly, no Dash, no I/O. Every public function below takes a `SceneState`
...
(per PLAN.md §8 caveats G2 and G5):

  * `_view_to_body` — the 4-line ZYX-transpose that mirrors the private
    `radiant.source.shapes._helpers.view_to_body`. Rule 11 (no cross-stage
    private imports) and the GUI's "no `_underscored` symbols" rule make
    re-implementation the right call.
...
C3 invariant (PLAN.md §3): `projected_area_m2(state)` is computed by calling
`TargetShape.projected_area(...)` directly. There is no parallel implementation
of projected area anywhere in this tool.
"""
...
def build_target_shape(state: SceneState) -> TargetShape:
    ...
    shape avoids double-rotating. This is the convention PLAN.md §8 G5
    locks in: the view-model performs the rotation; the shape consumes a
    body-frame vector.
    """
...
def view_direction_body(state: SceneState) -> npt.NDArray[np.float64]:
    ...
    locally (PLAN.md §8 G5). `R_target` rotates body→scene; its transpose
    rotates scene→body. The shapes were instantiated with `orientation_rad=
    (0, 0, 0)` (see `build_target_shape`), so the body-frame vector returned
    here flows straight into `shape.projected_area` without further rotation.
    """
...
def classify_regime(state: SceneState) -> tuple[RadiometricRegime, str]:
    ...
    Phase 5 readout shows verbatim.
    """
...
def derived_readout(state: SceneState) -> dict[str, tuple[float, str]]:
    """Labeled scalars + units strings for the Phase 5 readout panel.
...
# unit-bearing without sprinkling format strings through main.py.
_READOUT_FORMATTERS: Final[dict[str, tuple[str, "callable[[float], str]"]]] = {  # type: ignore[name-defined]
    # km is friendlier than meters for orbital ranges.
    ...
    # angular_extent can be +inf (degenerate stack-up); guard explicitly.
    "ro-angular-extent": (
        "µrad",
        lambda v: "∞ rad" if not math.isfinite(v) else f"{v * 1e6:.3f} µrad",
    ),
    "ro-fill-fraction": ("(dimensionless)", lambda v: f"{v:.3f}"),
...
def format_readout(
    ...
    Sole formatter for the Phase-5 readout panel. Numeric rows pull from
    ...
    for component_id, (unit_label, formatter) in _READOUT_FORMATTERS.items():
        ...
        # ro-fill-fraction → derived key is `fill_fraction_effective`
...
# ---------------------------------------------------------------------------
# Multi-facet explainer (Phase-5 prompt §"Goal" item 4)
# ---------------------------------------------------------------------------

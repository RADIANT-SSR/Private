# GUI scripting-window script — Scenario 4.3: Camouflage effectiveness — bare vehicle, LWIR FLIR at nadir
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/4.3_camouflage_effectiveness.gui.yaml
...
#
# NOTE: Bare oxidized-steel vehicle vs scrub, LWIR FLIR at nadir. The YAML references a derived radiance CSV regenerated by importing the runner.
#
...
# (headless smoke test): python scripts/gui_console_4.3_camouflage_effectiveness.py
...
try:
    ...
    sensor = Sensor.from_yaml(
        _Path(__file__).resolve().parent.parent / "inputs" / "4.3_camouflage_effectiveness.gui.yaml"
    )
...
print("=== Scenario 4.3: Camouflage effectiveness — bare vehicle, LWIR FLIR at nadir ===")
...
def _sweep_and_plot():
    center = 0.1  # current Aperture diameter [m]
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 4.3: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

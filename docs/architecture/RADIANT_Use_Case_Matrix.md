...
8. **B2 / B4 boundary for near-horizontal paths** (new, 2026-07-26). Rule B distinguishes "exits to space through an atmospheric column" (B2, supported) from "limb-crossing" (B4, raises). The exact geometric test — presumably on the tangent height of the forward continuation relative to the endpoint altitudes — is a Phase 2 design item; it must not classify a level air-to-air path as a limb crossing, or the ratified air-to-air class becomes unreachable. Interacts with the guard band of decision 18.
...
10. **Does the ±0.5° hard guard apply to constant-altitude arms?** (new, 2026-07-26). Decision 18 raises for any LOS within ±0.5° of horizontal, but a short A5 arm is horizontal by construction: two 30 m towers 8 km apart sit at θ_o ≈ 90.04°, well inside the guard, while being a benign short path with no limb traversal and negligible refraction bending. Taken literally the guard would reject the ground-to-ground and short air-to-air classes it was never aimed at. The guard's discriminator is therefore probably not $|\theta_o - \pi/2|$ alone but the tangent-height depression of the path below its endpoints (equivalently, path length at altitude) — a Phase 1 design item that must be settled before the horizontal solution lands. Interacts with question 8.

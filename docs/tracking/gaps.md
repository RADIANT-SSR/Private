...
| **Status** | OPEN — **item 1 DELIVERED 2026-09-07** (`calibration.source_uniformity_K` + `cal_source_uniformity` term, `source_uniformity.py`, Level 0 + stage + chain tests, GUI Source-group field; commit on `gap122/item1-source-uniformity`); items 2–5 on demand. |
...
| **Status** | **DELIVERED 2026-09-08** (commit `40a0c9f0`, branch `gap126/shipped-examples`, merged after owner review; wheel-verified in a scratch venv — 6 templates + 6 examples discovered off-repo, the CSV-referencing OLI-2 study loads and evaluates from site-packages). |

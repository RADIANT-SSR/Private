...
| **Detector** | FPA part library (named presets) | **[SHIPPED — Gap 119 plan Phase 3]** `FPAPartSelector` — pick a real part (21 shipped presets, grouped by class) and apply it in one `sensor.apply_fpa` call; blurb shows band + basis census (datasheet/paper/derived/assumed counts) before applying; report row summarizes applied-vs-kept (presets seed, explicit wins) with a per-parameter Details dialog; Open datasheet/paper opens the committed PDF (`docs/validation/fpa_datasheets/`) in a repo checkout, else the citation URL/DOI. Part metadata via `radiant.api.fpa_preset.available_fpa_parts` (GUI never imports `radiant.data`). Sits above the Inputs card in the Detector **Inputs** tab; a successful apply re-enters the debounced re-evaluation path |

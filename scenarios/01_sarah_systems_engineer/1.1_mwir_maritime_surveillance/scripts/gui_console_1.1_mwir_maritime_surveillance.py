# GUI scripting-window script — Scenario 1.1: MWIR maritime surveillance — aperture trade (30 cm baseline)
#
# HOW TO USE
#   1. In the RADIANT GUI: File -> Open YAML -> inputs/1.1_mwir_maritime_surveillance.gui.yaml
...
#
# NOTE: Baseline = 30 cm aperture, SimpleAtmosphere (maritime aerosol). The MODTRAN-D2 tape7 variant needs a staged file; the console script shows how to switch to it.
#
...
# (headless smoke test): python scripts/gui_console_1.1_mwir_maritime_surveillance.py
...
try:
    ...
    sensor = Sensor.load(
        _Path(__file__).resolve().parent.parent / "inputs" / "1.1_mwir_maritime_surveillance.gui.yaml"
    )
...
print("=== Scenario 1.1: MWIR maritime surveillance — aperture trade (30 cm baseline) ===")
...
def _sweep_and_plot():
    ...
    ax.set_ylabel("SNR [-]")
    ax.set_title("Scenario 1.1: SNR vs aperture diameter")
    ax.grid(alpha=0.3)
    return fig

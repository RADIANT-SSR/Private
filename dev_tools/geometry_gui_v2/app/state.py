...
slider-display units (deg, km, µm) happens in the Qt callback layer in
later phases — never inside this file or ``view_model.py``.

Lifted verbatim from v1 (``dev_tools/geometry_gui/app/state.py``) per
PLAN_v2.md §6 (salvage list). No content changes.
"""
...
class SceneState:
    ...
    Phases 2+ derive every PyVista actor, every readout value, and every
...
def default_state() -> SceneState:
    """Convenience alias for ``SceneState.default()``.

    Referenced by PLAN_v2.md §9 acceptance command::

        python -c "from dev_tools.geometry_gui_v2.scene import build_scene; \\
                   from dev_tools.geometry_gui_v2.app.state import default_state; \\
                   p = build_scene(default_state()); p.show()"
    """
